...
class DashScopeEmbeddingClient:
    ...
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None):
        ...
        self.model = model or os.getenv("DASHSCOPE_EMBEDDING_MODEL") or getattr(config, 'DASHSCOPE_EMBEDDING_MODEL', 'text-embedding-v4')
        ...
        logger.info(
            f"[DashScopeEmbedding] Initialized with model={self.model} "
            f"(multimodal={self.is_multimodal}, OpenAI-compatible={self.use_openai_compatible})"
        )
    ...
    def _make_request(
        ...
        for i, text in enumerate(texts):
            if len(text) > max_chars:
                logger.debug(f"[DashScopeEmbedding] Text {i} length {len(text)} chars exceeds {max_tokens_per_text} token limit, truncating")
        ...
        if self.use_openai_compatible:
            ...
            if instruct and self.model.startswith("text-embedding-v4"):
                if input_type != "query":
                    logger.warning("[DashScopeEmbedding] instruct parameter requires input_type='query', ignoring instruct")
                else:
                    payload["parameters"]["instruct"] = instruct
            
            # Add output_type (for v3, v4)
            if output_type != "dense" and (self.model.startswith("text-embedding-v3") or self.model.startswith("text-embedding-v4")):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                with httpx.Client(timeout=60.0) as client:
                    ...
                    if not success and error:
                        ...
                        if should_retry(error, attempt, max_retries):
                            ...
                            logger.warning(
                                f"[DashScopeEmbedding] Error on attempt {attempt}/{max_retries}: {error.message}. "
                ...
                for i, embedding in enumerate(raw_embeddings):
                    try:
                        ...
                        if np.isnan(embedding_array).any() or np.isinf(embedding_array).any():
                            logger.warning(
                                f"[DashScopeEmbedding] Invalid embedding (NaN/Inf) at index {i}, "
                        ...
                        if norm > 0:
                            ...
                            logger.warning(
                                f"[DashScopeEmbedding] Zero-norm embedding at index {i}, "
                                f"skipping this embedding"
                            )
                            continue
                            
                    except Exception as e:
                        logger.error(f"[DashScopeEmbedding] Failed to normalize embedding at index {i}: {e}")
                        continue
                
                if len(normalized_embeddings) != len(raw_embeddings):
                    logger.warning(
                        f"[DashScopeEmbedding] Normalized {len(normalized_embeddings)}/{len(raw_embeddings)} "
                        f"embeddings (some were invalid)"
                    )
                ...
                if should_retry(e, attempt, max_retries):
                    ...
                    logger.warning(
                        f"[DashScopeEmbedding] DashScope error on attempt {attempt}/{max_retries}: {e.message}. "
                        f"Retrying in {delay}s..."
                    )
                    time.sleep(delay)
                    continue
                    ...
                    logger.error(f"[DashScopeEmbedding] DashScope API error: {e.message} (code: {e.error_code}, type: {e.error_type})")
                    raise
                ...
                if should_retry(last_error, attempt, max_retries):
                    ...
                    logger.warning(
                        f"[DashScopeEmbedding] HTTP error on attempt {attempt}/{max_retries}: {e}. "
                        f"Retrying in {delay}s..."
                    )
                    time.sleep(delay)
                    continue
                else:
                    logger.error(f"[DashScopeEmbedding] HTTP error: {e}")
                    raise
            except Exception as e:
                # Other errors - don't retry
                logger.error(f"[DashScopeEmbedding] Error embedding texts: {e}")
                raise
    ...
    def embed_texts(
        ...
        for i in range(0, len(texts), self.batch_size):
            ...
            try:
                ...
                logger.debug(f"[DashScopeEmbedding] Embedded batch {i//self.batch_size + 1}, size={len(batch)}")
            except Exception as e:
                logger.error(f"[DashScopeEmbedding] Failed to embed batch starting at index {i}: {e}")
                raise
    ...
    def embed_query(
        ...
        try:
            ...
            logger.error(f"[DashScopeEmbedding] Failed to embed query: {e}")
            raise
    ...
    def embed_image(self, image_path: str) -> List[float]:
        ...
        try:
            ...
            logger.error(f"[DashScopeEmbedding] Failed to embed image: {e}")
            raise
    ...
    def _make_multimodal_request(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # Other errors - don't retry
                logger.error(f"[DashScopeEmbedding] Error embedding multimodal content: {e}")
                raise
