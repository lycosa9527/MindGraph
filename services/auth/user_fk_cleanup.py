...
async def _null_token_usage_user_id(db: AsyncSession, user_id: int) -> None:
    if TokenUsage is None:
        return
    await db.execute(
        update(TokenUsage).where(TokenUsage.user_id == user_id).values(user_id=None)
    )
...
async def _delete_community_for_user(db: AsyncSession, user_id: int) -> None:
    ...
    await db.execute(
        delete(CommunityPostComment).where(CommunityPostComment.post_id.in_(ap))
    )
    await db.execute(
        delete(CommunityPostLike).where(CommunityPostLike.post_id.in_(ap))
    )
    await db.execute(
        delete(CommunityPostLike).where(CommunityPostLike.user_id == user_id)
    )
    await db.execute(
        delete(CommunityPostComment).where(CommunityPostComment.user_id == user_id)
    )
...
async def _delete_school_zone_for_user(db: AsyncSession, user_id: int) -> None:
    dsub = (
        select(SharedDiagram.id).where(SharedDiagram.author_id == user_id).scalar_subquery()
    )
    await db.execute(
        delete(SharedDiagramComment).where(SharedDiagramComment.diagram_id.in_(dsub))
    )
    await db.execute(
        delete(SharedDiagramLike).where(SharedDiagramLike.diagram_id.in_(dsub))
    )
    await db.execute(
        delete(SharedDiagramLike).where(SharedDiagramLike.user_id == user_id)
    )
    await db.execute(
        delete(SharedDiagramComment).where(SharedDiagramComment.user_id == user_id)
    )
    await db.execute(
        delete(SharedDiagram).where(SharedDiagram.author_id == user_id)
    )


async def _delete_library_for_user(db: AsyncSession, user_id: int) -> None:
    await db.execute(
        delete(LibraryDanmakuLike).where(LibraryDanmakuLike.user_id == user_id)
    )
    await db.execute(
        delete(LibraryDanmakuReply).where(LibraryDanmakuReply.user_id == user_id)
    )
    await db.execute(
        delete(LibraryDanmaku).where(LibraryDanmaku.user_id == user_id)
    )
    await db.execute(
        delete(LibraryBookmark).where(LibraryBookmark.user_id == user_id)
    )
    await db.execute(
        delete(LibraryDocument).where(LibraryDocument.uploader_id == user_id)
    )
...
async def _delete_debate_for_user(db: AsyncSession, user_id: int) -> None:
    ...
    await db.execute(
        text(
            "DELETE FROM debate_judgments WHERE session_id IN "
            "(SELECT id FROM debate_sessions WHERE user_id = :uid)"
        ),
        uid,
    )
    await db.execute(
        text(
            "DELETE FROM debate_messages WHERE session_id IN "
            "(SELECT id FROM debate_sessions WHERE user_id = :uid)"
        ),
        uid,
    )
    await db.execute(
        text(
            "DELETE FROM debate_participants WHERE session_id IN "
            "(SELECT id FROM debate_sessions WHERE user_id = :uid)"
        ),
        uid,
    )
    await db.execute(
        text("DELETE FROM debate_sessions WHERE user_id = :uid"), uid
    )
    await db.execute(
        text(
    ...
    await db.execute(
        text("DELETE FROM debate_participants WHERE user_id = :uid"), uid
    )
...
async def _delete_workshop_for_user(db: AsyncSession, user_id: int) -> None:
    ...
    await db.execute(
        text("DELETE FROM chat_messages WHERE sender_id = :uid"), uid
    )
    ...
    for _ in range(64):
        ...
        logger.warning(
            "[UserFkCleanup] channel leaf loop did not finish for user %s", user_id
        )
    rem = await db.execute(
        text("SELECT 1 FROM chat_channels WHERE created_by = :uid LIMIT 1"), uid
    )
    if rem.first():
        await db.execute(
            text("DELETE FROM chat_channels WHERE created_by = :uid"), uid
        )
...
async def delete_user_fk_dependent_rows(db: AsyncSession, user_id: int) -> None:
    ...
    await db.execute(
        update(Device).where(Device.student_id == user_id).values(student_id=None)
    )
    await db.execute(
        delete(UserAPIToken).where(UserAPIToken.user_id == user_id)
    )
    await db.execute(
        delete(UpdateNotificationDismissed).where(
            UpdateNotificationDismissed.user_id == user_id
        )
    )
    await db.execute(
        delete(FeatureAccessUserGrant).where(
            FeatureAccessUserGrant.user_id == user_id
        )
    )
    await db.execute(
        delete(PinnedConversation).where(PinnedConversation.user_id == user_id)
    )
    await db.execute(
        delete(UserUsageStats).where(UserUsageStats.user_id == user_id)
    )
    await db.execute(
    ...
    ks = (
        await db.execute(
            select(KnowledgeSpace).where(KnowledgeSpace.user_id == user_id)
        )
    ).scalar_one_or_none()
    if ks is not None:
        await db.delete(ks)
