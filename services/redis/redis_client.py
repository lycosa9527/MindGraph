    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(_RETRY_MAX_ATTEMPTS):
                ...
                except (redis_module.ConnectionError, redis_module.TimeoutError) as e:  # type: ignore[attr-defined]
                    last_error = e
...
class RedisOperations:
    ...
    def get_and_delete(key: str) -> Optional[str]:
        ...
        if not _redis_available or not _redis_client:
            logger.debug(f"[Redis] get_and_delete: Redis unavailable for key: {key}")
            return None
        try:
            ...
            logger.warning(f"[Redis] get_and_delete failed for key {key}: {e}")
            return None
