...
class RedisTokenBuffer:
    ...
    MODEL_PRICING = {
        ...
        "qwen3.8-flash": {"input": 0.3, "output": 0.6, "provider": "dashscope"},
    ...
    async def _push_record(self, record: Dict) -> bool:
        """Push record to the Redis stream (XADD).

        On Redis 8.6+ the app enforces a compatible redis (redis-py) at startup, so
        the stream uses IDMPAUTO (``idmpauto=`` + ``id="*"``) when
        :attr:`_RedisCapabilities.idmpauto` is set. On older servers this flag is
        false and a standard ``XADD`` with autogenerated ids is used.

        ``STREAM_HARD_CAP`` is applied with ``MAXLEN ~`` (approximate trim).
        """
        if self._use_redis():
            try:
                redis = get_async_redis()
