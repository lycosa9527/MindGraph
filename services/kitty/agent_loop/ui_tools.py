"""Kitty UI tool schemas for the typed loop (general mode) and command mapping.
...
def build_ui_diagram_tools() -> List[Dict[str, Any]]:
    """OpenAI-compatible UI tool definitions for the typed Kitty loop."""
    ...
    return [
        ...
        fn(
            ...
            "Update an existing node's text.",
            {
                "node_identifier": {
                    "type": "string",
                    "description": "Node text, outline no (1 / 1.1 / 2.1), or index label",
                },
...
def ui_tool_call_to_command(name: str, arguments_json: str) -> Dict[str, Any]:
    """Map a UI / diagram tool call to the legacy command dict."""
    ...
    if name == "update_node":
        ...
        cmd = {"action": "update_node", "confidence": 0.95}
...
build_omni_diagram_tools = build_ui_diagram_tools
omni_function_call_to_command = ui_tool_call_to_command
