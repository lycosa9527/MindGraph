...
def _parse_400_extended_errors(
    ...
    if error_code.startswith('InvalidFile.') or 'invalid file' in error_msg_lower:
        ...
        if 'download' in error_msg_lower and ('failed' in error_msg_lower or 'timeout' in error_msg_lower):
            user_msg = "文件下载失败或超时，请检查文件URL是否可访问" if has_chinese else "File download failed or timeout. Please check if file URL is accessible."
            return LLMProviderError(f"File download error: {error_message}", provider='dashscope', error_code=error_code or 'InvalidFile.DownloadFailed'), user_msg

        # File format errors
        if 'format' in error_msg_lower and ('not supported' in error_msg_lower or 'illegal' in error_msg_lower):
            user_msg = "文件格式不支持，请检查文件格式" if has_chinese else "File format not supported. Please check file format."
            return LLMInvalidParameterError(f"Invalid file format: {error_message}", parameter='file', error_code=error_code or 'InvalidFile.Format', provider='dashscope'), user_msg

        # File size errors
        if 'size' in error_msg_lower and ('exceed' in error_msg_lower or 'too large' in error_msg_lower):
            user_msg = "文件大小超出限制，请压缩文件" if has_chinese else "File size exceeds limit. Please compress file."
            return LLMInvalidParameterError(f"File too large: {error_message}", parameter='file', error_code=error_code or 'InvalidFile.Size', provider='dashscope'), user_msg

        # File duration errors
        if 'duration' in error_msg_lower and ('exceed' in error_msg_lower or 'too long' in error_msg_lower):
            user_msg = "文件时长超出限制，请缩短文件时长" if has_chinese else "File duration exceeds limit. Please shorten file duration."
            return LLMInvalidParameterError(f"File duration too long: {error_message}", parameter='file', error_code=error_code or 'InvalidFile.Duration', provider='dashscope'), user_msg

        # Audio length errors
        if 'audio length' in error_msg_lower or ('audio' in error_msg_lower and 'between' in error_msg_lower and 's' in error_msg_lower):
            user_msg = "音频时长不符合要求，请确保音频时长在指定范围内" if has_chinese else "Audio duration not in required range. Please ensure audio duration is within limits."
            return LLMInvalidParameterError(f"Invalid audio length: {error_message}", parameter='audio', error_code=error_code or 'InvalidFile.AudioLengthError', provider='dashscope'), user_msg

        # Image resolution errors
        if 'resolution' in error_msg_lower or ('image' in error_msg_lower and ('size' in error_msg_lower or 'dimension' in error_msg_lower)):
            user_msg = "图像分辨率不符合要求，请调整图像尺寸" if has_chinese else "Image resolution not in required range. Please adjust image size."
            return LLMInvalidParameterError(f"Invalid image resolution: {error_message}", parameter='image', error_code=error_code or 'InvalidFile.Resolution', provider='dashscope'), user_msg

        # Video FPS errors
        if 'fps' in error_msg_lower and 'must be' in error_msg_lower:
            user_msg = "视频帧率不符合要求，应在15-60fps之间" if has_chinese else "Video FPS not in required range (15-60fps)."
            return LLMInvalidParameterError(f"Invalid video FPS: {error_message}", parameter='video', error_code=error_code or 'InvalidFile.FPS', provider='dashscope'), user_msg

        # Human body detection errors
        if 'no human' in error_msg_lower or 'human body' in error_msg_lower:
            user_msg = "图片中未检测到人体，请上传包含人体的图片" if has_chinese else "No human body detected. Please upload image with human body."
            return LLMInvalidParameterError(f"No human body detected: {error_message}", parameter='image', error_code=error_code or 'InvalidFile.NoHuman', provider='dashscope'), user_msg

        # Face detection errors
        if 'face' in error_msg_lower and ('not detected' in error_msg_lower or 'invalid' in error_msg_lower):
            user_msg = "未检测到人脸或人脸不符合要求，请上传包含清晰人脸的图片" if has_chinese else "Face not detected or invalid. Please upload image with clear face."
            return LLMInvalidParameterError(f"Face detection error: {error_message}", parameter='image', error_code=error_code or 'InvalidFile.Face', provider='dashscope'), user_msg

        # File parsing errors
        if 'parsing' in error_msg_lower or 'parse' in error_msg_lower and 'failed' in error_msg_lower:
            user_msg = "文件解析失败，请检查文件是否损坏" if has_chinese else "File parsing failed. Please check if file is corrupted."
            return LLMProviderError(f"File parsing error: {error_message}", provider='dashscope', error_code=error_code or 'InvalidFile.ParseFailed'), user_msg

        # File not found
        if 'cannot be found' in error_msg_lower or 'not found' in error_msg_lower:
            user_msg = "文件不存在，请检查文件ID或URL" if has_chinese else "File not found. Please check file ID or URL."
            return LLMProviderError(f"File not found: {error_message}", provider='dashscope', error_code=error_code or 'InvalidFile.NotFound'), user_msg

        # File content blank
        if 'content blank' in error_msg_lower or 'content is empty' in error_msg_lower:
            user_msg = "文件内容为空，请确保文件内容不为空" if has_chinese else "File content is empty. Please ensure file has content."
            return LLMInvalidParameterError(f"File content blank: {error_message}", parameter='file', error_code=error_code or 'InvalidFile.Content', provider='dashscope'), user_msg

    # Audio-related errors (400-Audio.*)
    if error_code.startswith('Audio.') or ('audio' in error_msg_lower and ('error' in error_msg_lower or 'failed' in error_msg_lower)):
        # Audio decoder errors
        if 'decoder' in error_msg_lower or 'decode' in error_msg_lower:
            user_msg = "音频解码失败，请检查音频文件格式和编码" if has_chinese else "Audio decode failed. Please check audio file format and encoding."
            return LLMProviderError(f"Audio decode error: {error_message}", provider='dashscope', error_code=error_code or 'Audio.DecoderError'), user_msg

        # Audio sample rate errors
        if 'sample rate' in error_msg_lower or 'rate unsupported' in error_msg_lower:
            user_msg = "音频采样率不支持，采样率需大于等于24000 Hz" if has_chinese else "Audio sample rate unsupported. Sample rate must be >= 24000 Hz."
            return LLMInvalidParameterError(f"Invalid audio sample rate: {error_message}", parameter='audio', error_code=error_code or 'Audio.AudioRateError', provider='dashscope'), user_msg

        # Audio duration limit errors
        if 'duration exceeds' in error_msg_lower or 'duration limit' in error_msg_lower:
            user_msg = "音频时长超出限制，请缩短音频时长" if has_chinese else "Audio duration exceeds limit. Please shorten audio duration."
            return LLMInvalidParameterError(f"Audio duration limit exceeded: {error_message}", parameter='audio', error_code=error_code or 'Audio.DurationLimitError', provider='dashscope'), user_msg

        # Audio silent errors
        if 'silent' in error_msg_lower or 'no valid audio' in error_msg_lower:
            user_msg = "音频为静音或无效，请确保音频包含有效语音" if has_chinese else "Audio is silent or invalid. Please ensure audio contains valid speech."
            return LLMInvalidParameterError(f"Silent audio error: {error_message}", parameter='audio', error_code=error_code or 'Audio.AudioSilentError', provider='dashscope'), user_msg

        # Audio too short
        if 'too short' in error_msg_lower and 'audio' in error_msg_lower:
            user_msg = "音频时长过短，请确保音频时长在10-15秒之间" if has_chinese else "Audio too short. Please ensure audio duration is 10-15 seconds."
            return LLMInvalidParameterError(f"Audio too short: {error_message}", parameter='audio', error_code=error_code or 'Audio.AudioShortError', provider='dashscope'), user_msg

        # Audio preprocess errors
        if 'preprocess' in error_msg_lower:
            user_msg = "音频预处理失败，请检查音频质量和格式" if has_chinese else "Audio preprocessing failed. Please check audio quality and format."
            return LLMProviderError(f"Audio preprocess error: {error_message}", provider='dashscope', error_code=error_code or 'Audio.PreprocessError'), user_msg
    ...
    if error_code.startswith('InvalidImage.') or ('image' in error_msg_lower and 'invalid' in error_msg_lower):
        ...
        if 'image size' in error_msg_lower or ('size' in error_msg_lower and 'beyond limit' in error_msg_lower):
            user_msg = "图片大小超出限制，请调整图片尺寸" if has_chinese else "Image size exceeds limit. Please adjust image size."
            return LLMInvalidParameterError(f"Image size error: {error_message}", parameter='image', error_code=error_code or 'InvalidImage.ImageSize', provider='dashscope'), user_msg

        # Image format errors
        if 'format' in error_msg_lower and 'invalid' in error_msg_lower:
            user_msg = "图片格式无效，请使用支持的格式（JPEG, PNG, BMP, WEBP等）" if has_chinese else "Invalid image format. Please use supported formats (JPEG, PNG, BMP, WEBP, etc.)."
            return LLMInvalidParameterError(f"Invalid image format: {error_message}", parameter='image', error_code=error_code or 'InvalidImage.Format', provider='dashscope'), user_msg

        # Image resolution errors
        if 'resolution' in error_msg_lower and ('too large' in error_msg_lower or 'too small' in error_msg_lower):
            user_msg = "图片分辨率过大或过小，请调整图片分辨率" if has_chinese else "Image resolution too large or too small. Please adjust image resolution."
            return LLMInvalidParameterError(f"Invalid image resolution: {error_message}", parameter='image', error_code=error_code or 'InvalidImage.Resolution', provider='dashscope'), user_msg

        # No human face errors
        if 'no human face' in error_msg_lower or 'face not detected' in error_msg_lower:
            user_msg = "未检测到人脸，请上传包含清晰人脸的图片" if has_chinese else "No human face detected. Please upload image with clear face."
            return LLMInvalidParameterError(f"No human face detected: {error_message}", parameter='image', error_code=error_code or 'InvalidImage.NoHumanFace', provider='dashscope'), user_msg
    ...
    if error_code.startswith('InvalidURL.') or ('url' in error_msg_lower and 'invalid' in error_msg_lower):
        ...
        if 'connection refused' in error_msg_lower:
            user_msg = "URL连接被拒绝，请提供可用的URL" if has_chinese else "URL connection refused. Please provide available URL."
            return LLMProviderError(f"URL connection refused: {error_message}", provider='dashscope', error_code=error_code or 'InvalidURL.ConnectionRefused'), user_msg

        # URL timeout
        if 'timeout' in error_msg_lower and 'url' in error_msg_lower:
            user_msg = "URL下载超时，请检查网络连接" if has_chinese else "URL download timeout. Please check network connection."
            return LLMProviderError(f"URL timeout: {error_message}", provider='dashscope', error_code=error_code or 'InvalidURL.Timeout'), user_msg

        # Invalid URL format
        if 'does not appear to be valid' in error_msg_lower or 'url format' in error_msg_lower:
            user_msg = "URL格式无效，请确保URL格式正确（http://、https://或data:开头）" if has_chinese else "Invalid URL format. Ensure URL starts with http://, https://, or data:."
            return LLMInvalidParameterError(f"Invalid URL format: {error_message}", parameter='url', error_code=error_code or 'InvalidURL', provider='dashscope'), user_msg
    ...
    if error_code.startswith('BadRequest.') or 'bad request' in error_msg_lower:
        ...
        if 'empty input' in error_msg_lower or 'required input parameter missing' in error_msg_lower:
            user_msg = "缺少必需的input参数，请在请求中添加input参数" if has_chinese else "Missing required input parameter. Please add input parameter to request."
            return LLMInvalidParameterError(f"Empty input: {error_message}", parameter='input', error_code=error_code or 'BadRequest.EmptyInput', provider='dashscope'), user_msg

        # Empty parameters
        if 'empty parameters' in error_msg_lower or 'required parameter "parameters" missing' in error_msg_lower:
            user_msg = "缺少必需的parameters参数，请在请求中添加parameters参数" if has_chinese else "Missing required parameters. Please add parameters to request."
            return LLMInvalidParameterError(f"Empty parameters: {error_message}", parameter='parameters', error_code=error_code or 'BadRequest.EmptyParameters', provider='dashscope'), user_msg

        # Empty model
        if 'empty model' in error_msg_lower or 'required parameter "model" missing' in error_msg_lower:
            user_msg = "缺少必需的model参数，请在请求中添加model参数" if has_chinese else "Missing required model parameter. Please add model parameter to request."
            return LLMInvalidParameterError(f"Empty model: {error_message}", parameter='model', error_code=error_code or 'BadRequest.EmptyModel', provider='dashscope'), user_msg

        # Illegal input
        if 'illegal input' in error_msg_lower or 'input parameter requires json format' in error_msg_lower:
            user_msg = "输入参数格式错误，需要JSON格式" if has_chinese else "Input parameter format error. Requires JSON format."
            return LLMInvalidParameterError(f"Illegal input: {error_message}", parameter='input', error_code=error_code or 'BadRequest.IllegalInput', provider='dashscope'), user_msg

        # Input download failed
        if 'input download failed' in error_msg_lower or 'failed to download the input file' in error_msg_lower:
            user_msg = "输入文件下载失败，请检查文件URL" if has_chinese else "Input file download failed. Please check file URL."
            return LLMProviderError(f"Input download failed: {error_message}", provider='dashscope', error_code=error_code or 'BadRequest.InputDownloadFailed'), user_msg

        # Unsupported file format
        if 'unsupported file format' in error_msg_lower or 'file format unsupported' in error_msg_lower:
            user_msg = "文件格式不支持，请使用支持的文件格式" if has_chinese else "File format not supported. Please use supported file format."
            return LLMInvalidParameterError(f"Unsupported file format: {error_message}", parameter='file', error_code=error_code or 'BadRequest.UnsupportedFileFormat', provider='dashscope'), user_msg

        # Too large
        if 'too large' in error_msg_lower and 'payload' in error_msg_lower:
            user_msg = "文件大小超出限制，请压缩文件" if has_chinese else "File too large. Please compress file."
            return LLMInvalidParameterError(f"Payload too large: {error_message}", parameter='file', error_code=error_code or 'BadRequest.TooLarge', provider='dashscope'), user_msg

        # Resource not exist
        if 'resource not exist' in error_msg_lower or 'required resource not exist' in error_msg_lower:
            user_msg = "资源不存在，请检查资源ID" if has_chinese else "Resource not exist. Please check resource ID."
            return LLMProviderError(f"Resource not exist: {error_message}", provider='dashscope', error_code=error_code or 'BadRequest.ResourceNotExist'), user_msg

        # Voice not found
        if 'voice' in error_msg_lower and 'not found' in error_msg_lower:
            user_msg = "音色不存在，请检查音色ID" if has_chinese else "Voice not found. Please check voice ID."
            return LLMProviderError(f"Voice not found: {error_message}", provider='dashscope', error_code=error_code or 'BadRequest.VoiceNotFound'), user_msg
    ...
    if 'video' in error_msg_lower:
        ...
        if 'video modality' in error_msg_lower and 'does not meet' in error_msg_lower:
            user_msg = "视频输入不符合要求，请检查视频格式和参数" if has_chinese else "Video input does not meet requirements. Please check video format and parameters."
            return LLMInvalidParameterError(f"Invalid video modality: {error_message}", parameter='video', error_code=error_code or 'InvalidParameter.VideoModality', provider='dashscope'), user_msg

        # Video too long
        if 'video' in error_msg_lower and 'too long' in error_msg_lower:
            user_msg = "视频时长过长，请缩短视频时长" if has_chinese else "Video too long. Please shorten video duration."
            return LLMInvalidParameterError(f"Video too long: {error_message}", parameter='video', error_code=error_code or 'InvalidParameter.VideoDuration', provider='dashscope'), user_msg

        # Invalid video file
        if 'invalid video file' in error_msg_lower:
            user_msg = "视频文件无效，请检查视频文件是否损坏" if has_chinese else "Invalid video file. Please check if video file is corrupted."
            return LLMInvalidParameterError(f"Invalid video file: {error_message}", parameter='video', error_code=error_code or 'InvalidParameter.VideoFile', provider='dashscope'), user_msg

    # Multimodal content errors
    if 'multimodal' in error_msg_lower or 'content' in error_msg_lower and ('modal' in error_msg_lower or 'image_url' in error_msg_lower):
        ...
        if 'content must be a string' in error_msg_lower or 'input content must be a string' in error_msg_lower:
            user_msg = "纯文本模型不支持多模态内容，请将content设置为字符串" if has_chinese else "Text-only models do not support multimodal content. Set content to string."
            return LLMInvalidParameterError(f"Content must be string: {error_message}", parameter='content', error_code=error_code or 'InvalidParameter.ContentType', provider='dashscope'), user_msg

        # Missing image or text
        if 'lack of image or text' in error_msg_lower:
            user_msg = "缺少image或text字段，请添加image或text字段" if has_chinese else "Missing image or text field. Please add image or text field."
            return LLMInvalidParameterError(f"Missing image or text: {error_message}", parameter='content', error_code=error_code or 'InvalidParameter.MissingContent', provider='dashscope'), user_msg

    # File upload errors
    if 'file upload' in error_msg_lower or 'upload' in error_msg_lower and ('failed' in error_msg_lower or 'error' in error_msg_lower):
        user_msg = "文件上传失败，请检查文件大小和格式" if has_chinese else "File upload failed. Please check file size and format."
        return LLMProviderError(f"File upload error: {error_message}", provider='dashscope', error_code=error_code or 'InternalError.FileUpload'), user_msg
    ...
    if 'batch' in error_msg_lower:
        ...
        if 'not supported by the batch api' in error_msg_lower:
            user_msg = "当前模型不支持Batch调用，请使用其他调用方式" if has_chinese else "Model not supported for Batch API. Please use other call methods."
            return LLMInvalidParameterError(f"Batch not supported: {error_message}", parameter='model', error_code=error_code or 'ModelNotFound.Batch', provider='dashscope'), user_msg
        ...
        if 'mismatched_model' in error_msg_lower or 'does not match the rest of the batch' in error_msg_lower:
            user_msg = "Batch中所有请求必须使用同一模型" if has_chinese else "All requests in batch must use the same model."
            return LLMInvalidParameterError(f"Mismatched model in batch: {error_message}", parameter='model', error_code=error_code or 'InvalidParameter.BatchModel', provider='dashscope'), user_msg

        # Duplicate custom_id in batch
        if 'duplicate_custom_id' in error_msg_lower:
            user_msg = "Batch中请求ID重复，请确保每个请求ID唯一" if has_chinese else "Duplicate custom_id in batch. Ensure each request ID is unique."
            return LLMInvalidParameterError(f"Duplicate custom_id: {error_message}", parameter='custom_id', error_code=error_code or 'InvalidParameter.DuplicateId', provider='dashscope'), user_msg

    # Embedding errors
    if 'embedding' in error_msg_lower or 'batch size' in error_msg_lower:
        # Batch size invalid
        if 'batch size is invalid' in error_msg_lower:
            user_msg = "文本数量超过模型上限，请减少文本数量" if has_chinese else "Text count exceeds model limit. Please reduce text count."
            return LLMInvalidParameterError(f"Invalid batch size: {error_message}", parameter='input', error_code=error_code or 'InvalidParameter.BatchSize', provider='dashscope'), user_msg

        # Contents format error
        if 'contents is neither str nor list' in error_msg_lower:
            user_msg = "输入格式错误，应为字符串或字符串列表" if has_chinese else "Invalid input format. Should be string or list of strings."
            return LLMInvalidParameterError(f"Invalid contents format: {error_message}", parameter='input', error_code=error_code or 'InvalidParameter.Contents', provider='dashscope'), user_msg
    ...
    if 'total message token length exceed' in error_msg_lower or 'token length exceed' in error_msg_lower:
        user_msg = "输入总长度超过10,000,000 Token限制，请缩短输入内容" if has_chinese else "Total message token length exceeds 10,000,000 limit. Please shorten input."
        return LLMInvalidParameterError(f"Token length exceeded: {error_message}", parameter='messages', error_code=error_code or 'InvalidParameter.TokenLength', provider='dashscope'), user_msg
    ...
    if 'field required' in error_msg_lower or 'required parameter' in error_msg_lower:
        ...
        user_msg = f"缺少必需参数: {field_name or 'unknown'}" if has_chinese else f"Missing required parameter: {field_name or 'unknown'}"
        return LLMInvalidParameterError(
    ...
    if 'must be' in error_msg_lower and ('type' in error_msg_lower or 'integer' in error_msg_lower or 'float' in error_msg_lower):
        user_msg = f"参数类型错误: {error_message}" if has_chinese else f"Invalid parameter type: {error_message}"
        return LLMInvalidParameterError(
    ...
    if 'request parameter is invalid' in error_msg_lower or 'request parameter' in error_msg_lower and 'invalid' in error_msg_lower:
        user_msg = f"请求参数无效: {error_message}" if has_chinese else f"Invalid request parameter: {error_message}"
        return LLMInvalidParameterError(
    ...
    # JSON format errors
    if 'json' in error_msg_lower and ('error' in error_msg_lower or 'invalid' in error_msg_lower or 'format' in error_msg_lower):
        user_msg = "JSON格式错误，请检查JSON格式" if has_chinese else "Invalid JSON format. Please check JSON format."
        return LLMInvalidParameterError(
    ...
    if 'messages' in error_msg_lower:
        ...
        if 'length only support' in error_msg_lower or 'messages length' in error_msg_lower:
            user_msg = f"messages数组长度不符合要求: {error_message}" if has_chinese else f"Messages array length not supported: {error_message}"
            return LLMInvalidParameterError(
        ...
        if 'content length only support' in error_msg_lower:
            user_msg = f"content数组长度不符合要求: {error_message}" if has_chinese else f"Content array length not supported: {error_message}"
            return LLMInvalidParameterError(
        ...
        if 'last' in error_msg_lower and 'user' in error_msg_lower and 'message' in error_msg_lower:
            user_msg = "messages数组最后一位需为User Message" if has_chinese else "Last message in messages array must be User Message."
            return LLMInvalidParameterError(
