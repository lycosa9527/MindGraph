...
def parse_401_errors(
    ...
    if 'invalid api' in error_msg_lower or 'api.*key' in error_msg_lower or error_code in ['InvalidApiKey', 'invalid_api_key']:
        user_msg = "API密钥无效，请检查API密钥配置。常见原因：环境变量读取错误、密钥填写错误、地域不匹配" if has_chinese else "Invalid API key. Common causes: environment variable read error, incorrect key, region mismatch."
...
def parse_403_errors(
    ...
    if error_code == 'Model.AccessDenied' or 'model access denied' in error_msg_lower:
        user_msg = "无权访问此模型，请检查模型权限或申请模型访问权限" if has_chinese else "Access denied to model. Please check model permissions or apply for access."
        return LLMAccessDeniedError(
    ...
    if error_code == 'App.AccessDenied' or 'app access denied' in error_msg_lower:
        user_msg = "无权访问应用，请检查应用权限和发布状态" if has_chinese else "Access denied to app. Please check app permissions and publish status."
        return LLMAccessDeniedError(
    ...
    if error_code == 'Workspace.AccessDenied' or 'workspace access denied' in error_msg_lower:
        user_msg = "无权访问工作空间，请检查工作空间权限" if has_chinese else "Access denied to workspace. Please check workspace permissions."
        return LLMAccessDeniedError(
    ...
    # Free tier only
    if error_code == 'AllocationQuota.FreeTierOnly' or 'free tier' in error_msg_lower and 'exhausted' in error_msg_lower:
        user_msg = "免费额度已用完，如需付费调用请关闭'免费额度用完即停'模式" if has_chinese else "Free tier exhausted. To continue with paid calls, disable 'free tier only' mode."
    ...
    if 'policy expired' in error_msg_lower:
        user_msg = "文件上传凭证已过期，请重新生成凭证" if has_chinese else "File upload credential expired. Please regenerate credential."
