...
def parse_404_errors(
    ...
    # Model not found
    if error_code == 'ModelNotFound' or error_code == 'model_not_found' or ('model' in error_msg_lower and ('not' in error_msg_lower and 'found' in error_msg_lower)):
        user_msg = "模型不存在，请检查模型名称是否正确（注意大小写）或是否已开通服务" if has_chinese else "Model not found. Please check model name (case-sensitive) or if service is enabled."
    ...
    if error_code == 'model_not_supported' or 'unsupported model' in error_msg_lower and 'openai compatibility' in error_msg_lower:
        user_msg = "当前模型不支持OpenAI兼容方式接入，请使用DashScope原生方式调用" if has_chinese else "Model not supported for OpenAI compatibility. Please use DashScope native API."
