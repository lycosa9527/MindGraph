...
def parse_specialized_errors(
    ...
    if status_code == 409:
        if 'already exists' in error_msg_lower and 'model instance' in error_msg_lower:
            user_msg = "已存在重名的部署实例，请指定不同的后缀名" if has_chinese else "Model instance already exists. Please specify different suffix."
    ...
    if error_code in ['Throttling.AllocationQuota', 'Arrearage', 'insufficient_quota'] and status_code not in [403, 429]:
    ...
    if 'websocket' in error_msg_lower or 'ws' in error_msg_lower:
        ...
        if 'invalid payload' in error_msg_lower:
            user_msg = "WebSocket payload数据格式错误，请检查JSON格式" if has_chinese else "Invalid WebSocket payload format. Please check JSON format."
            return LLMInvalidParameterError(
        ...
        if 'timeout' in error_msg_lower and 'connection' in error_msg_lower:
            user_msg = "WebSocket连接超时，请检查网络连接和防火墙设置" if has_chinese else "WebSocket connection timeout. Please check network and firewall settings."
            return LLMTimeoutError(f"WebSocket connection timeout: {error_message}"), user_msg

        # Client disconnect
        if 'client disconnected' in error_msg_lower:
            user_msg = "客户端在任务结束前断开连接，请检查代码逻辑" if has_chinese else "Client disconnected before task finished. Please check code logic."
            return LLMServiceError(f"Client disconnected: {error_message}"), user_msg

        # Message too big
        if 'message was too big' in error_msg_lower:
            user_msg = "WebSocket消息过大，请分段发送数据" if has_chinese else "WebSocket message too big. Please send data in segments."
            return LLMInvalidParameterError(
    ...
    if 'audio' in error_msg_lower or 'video' in error_msg_lower:
        ...
        if 'unsupported audio format' in error_msg_lower:
            user_msg = "音频格式不支持，请使用WAV（16bit）、MP3或M4A格式" if has_chinese else "Unsupported audio format. Please use WAV (16bit), MP3, or M4A."
            return LLMInvalidParameterError(
        ...
        if 'no valid audio' in error_msg_lower or 'no valid audio error' in error_msg_lower:
            user_msg = "未检测到有效语音，请检查音频输入和格式" if has_chinese else "No valid audio detected. Please check audio input and format."
            return LLMInvalidParameterError(
    ...
    if 'flow' in error_msg_lower:
        ...
        if error_code == 'FlowNotPublished' or 'flow has not published' in error_msg_lower:
            user_msg = "流程未发布，请发布流程后再重试" if has_chinese else "Flow not published. Please publish flow and retry."
    ...
    if 'schema' in error_msg_lower:
        ...
        if error_code == 'InvalidSchema' or 'database schema is invalid' in error_msg_lower:
            user_msg = "数据库Schema无效，请输入数据库Schema信息" if has_chinese else "Invalid database schema. Please input database schema information."
            return LLMInvalidParameterError(
        ...
        if error_code == 'InvalidSchemaFormat' or 'schema format is invalid' in error_msg_lower:
            user_msg = "数据库Schema格式错误，请检查并修正数据表信息格式" if has_chinese else "Invalid schema format. Please check and correct data table format."
            return LLMInvalidParameterError(
    ...
    if error_code == 'UnsupportedOperation' or 'unsupported operation' in error_msg_lower:
        user_msg = "不支持的操作，请检查操作对象和操作类型是否匹配" if has_chinese else "Unsupported operation. Please check if operation object and type match."
        return LLMProviderError(
    ...
    if error_code == 'ServiceUnavailableError' or 'service unavailable' in error_msg_lower:
        user_msg = "服务不可用，请检查服务状态" if has_chinese else "Service unavailable. Please check service status."
