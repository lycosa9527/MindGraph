...
def parse_content_filter_errors(
    ...
    if error_code == 'Arrearage' or ('arrearage' in error_msg_lower and 'access denied' in error_msg_lower):
        user_msg = "账户欠费，访问被拒绝。请前往费用与成本查看并充值" if has_chinese else "Account in arrears, access denied. Please check billing and recharge."
    ...
    if error_code in ['DataInspectionFailed', 'data_inspection_failed', 'DataInspection', 'InvalidParameter.DataInspection'] or \
       'inappropriate content' in error_msg_lower or 'data inspection' in error_msg_lower:
        if 'input' in error_msg_lower and 'output' in error_msg_lower:
            user_msg = "输入或输出可能包含不当内容，请修改输入内容" if has_chinese else "Input or output may contain inappropriate content. Please modify input."
        elif 'input' in error_msg_lower:
            user_msg = "输入可能包含不当内容，请修改输入内容" if has_chinese else "Input may contain inappropriate content. Please modify input."
        elif 'output' in error_msg_lower:
            user_msg = "输出可能包含不当内容，请修改输入后重试" if has_chinese else "Output may contain inappropriate content. Please modify input and retry."
        else:
    ...
    # 400 IP Infringement
    if error_code == 'IPInfringementSuspect' or 'ip infringement' in error_msg_lower or 'intellectual property' in error_msg_lower:
    ...
    if error_code == 'CustomRoleBlocked' or 'custom rule' in error_msg_lower or 'custom role blocked' in error_msg_lower:
