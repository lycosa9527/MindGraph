...
from services.rate_limiter import LoadBalancerRateLimiter, DashscopeRateLimiter
...
class LLMLoadBalancer:
    ...
    FIXED_MODEL_MAP = {
        ...
        'kimi': 'ark-kimi',         # → Volcengine Kimi via endpoint (ALWAYS - 5,000 RPM, 500,000 TPM vs Dashscope's 60 RPM, 100,000 TPM)
    ...
    def __init__(
        ...
        if strategy not in valid_strategies:
            logger.warning(
                f"[LoadBalancer] Invalid strategy '{strategy}', "
                f"must be one of {valid_strategies}. Using 'round_robin'."
            )
            self.strategy = 'round_robin'
        
        logger.info(
            f"[LoadBalancer] Initialized: strategy={self.strategy}, "
            f"weights={self.weights}, enabled={enabled}, redis={self._use_redis}"
        )
    ...
    def _select_deepseek_provider(self) -> str:
        ...
        if self.strategy == 'weighted':
            ...
            logger.debug(f"[LoadBalancer] Weighted selection: rand={rand}, provider={provider}")
            return provider
            ...
            if self._use_redis:
                ...
                if counter is not None:
                    ...
                    logger.debug(f"[LoadBalancer] Round-robin (Redis): counter={counter}, provider={provider}")
                    return provider
                else:
                    # Redis failed, fall back to local counter
                    logger.warning("[LoadBalancer] Redis unavailable, using local counter for round-robin (uneven distribution in multi-worker)")
            ...
            logger.debug(f"[LoadBalancer] Round-robin (local): counter={self._counter}, provider={provider}")
            return provider
    ...
    def map_model(self, logical_model: str) -> str:
        ...
        if logical_model == 'deepseek':
            ...
            if provider == self.PROVIDER_VOLCENGINE:
                ...
                physical = 'deepseek'
            logger.debug(f"[LoadBalancer] map_model: {logical_model} → {physical} (provider={provider})")
            return physical
        ...
        if logical_model == 'kimi' and physical != 'ark-kimi':
            logger.warning(
                f"[LoadBalancer] Kimi mapped to {physical} instead of ark-kimi! "
                f"Force routing to Volcengine (5,000 RPM vs Dashscope's 60 RPM)"
            )
            physical = 'ark-kimi'
        
        logger.debug(f"[LoadBalancer] map_model: {logical_model} → {physical} (fixed)")
        return physical
    ...
    def record_provider_metrics(
        ...
        try:
            ...
            logger.debug(f"[LoadBalancer] Failed to record metrics in Redis: {e}")
    ...
    def get_provider_health(self, provider: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"[LoadBalancer] Failed to get provider health from Redis: {e}")
            return {
                'success_rate': 1.0,
                'avg_duration': 0.0,
                'total_requests': 0,
    ...
    def _select_health_aware_provider(self) -> str:
        ...
        if dashscope_health['healthy'] and volcengine_health['healthy']:
            rand = random.randint(1, 100)
            provider = self.PROVIDER_DASHSCOPE if rand <= self.weights.get('dashscope', 50) else self.PROVIDER_VOLCENGINE
            logger.debug(f"[LoadBalancer] Health-aware: both healthy, using weighted: {provider}")
            return provider
        
        # If one is unhealthy, prefer the healthy one
        if dashscope_health['healthy'] and not volcengine_health['healthy']:
            logger.warning(
                f"[LoadBalancer] Health-aware: Volcengine unhealthy "
                f"(success_rate={volcengine_health['success_rate']:.2%}, "
                f"recent_failures={volcengine_health['recent_failures']}), "
                f"routing to Dashscope"
            )
            return self.PROVIDER_DASHSCOPE
        
        if volcengine_health['healthy'] and not dashscope_health['healthy']:
            logger.warning(
                f"[LoadBalancer] Health-aware: Dashscope unhealthy "
                f"(success_rate={dashscope_health['success_rate']:.2%}, "
                f"recent_failures={dashscope_health['recent_failures']}), "
                f"routing to Volcengine"
            )
            return self.PROVIDER_VOLCENGINE
        
        # Both unhealthy - use weighted selection (better than random)
        logger.warning(
            "[LoadBalancer] Health-aware: Both providers unhealthy, using weighted selection"
        )
        rand = random.randint(1, 100)
        provider = self.PROVIDER_DASHSCOPE if rand <= self.weights.get('dashscope', 50) else self.PROVIDER_VOLCENGINE
        return provider
