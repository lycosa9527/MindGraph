...
KITTY_VOICE_INTENT_ROWS: List[IntentRow] = [
    {
        "name": "update_center",
        "kind": "diagram",
        "channel": "hub_patch",
...
KITTY_VOICE_SPECIAL_FLOWS: List[Dict[str, Any]] = [
    {
        "name": "paragraph_path",
        "channel": "mixed",
...
def voice_intent_rows_as_json() -> List[Dict[str, Any]]:
    ...
    return [dict(r) for r in KITTY_VOICE_INTENT_ROWS]


def special_flows_as_json() -> List[Dict[str, Any]]:
    return [dict(s) for s in KITTY_VOICE_SPECIAL_FLOWS]
