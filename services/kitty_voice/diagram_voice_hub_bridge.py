...
def _preview_circle_map_update_center(command: Dict[str, Any], diagram_data: Dict[str, Any]) -> bool:
...
def _preview_circle_map_update_node(
    command: Dict[str, Any],
    diagram_data: Dict[str, Any],
    diagram_type: str,
) -> bool:
...
def _preview_circle_map_add_node(command: Dict[str, Any], diagram_data: Dict[str, Any]) -> bool:
    target = command.get("target")
    if not isinstance(target, str) or not target.strip():
        return False
...
def _preview_circle_map_delete_node(command: Dict[str, Any], diagram_data: Dict[str, Any]) -> bool:
    target = command.get("target")
...
def preview_voice_context_after_diagram_intent(
    *,
    ...
    Models **circle_map** for all four diagram intents and **double_bubble_map**
    ``update_center`` when ``left`` and ``right`` are set on ``command``.
    Returns ``None`` when this preview does not cover the pair (see runtime handlers).
    """
    ...
    if action == "update_center":
        if _preview_circle_map_update_center(command, diagram_data):
            return out
        return None
    if action == "update_node":
        if _preview_circle_map_update_node(command, diagram_data, diagram_type):
            return out
        return None
    if action == "add_node":
        if _preview_circle_map_add_node(command, diagram_data):
            return out
        return None
    if action == "delete_node":
        if _preview_circle_map_delete_node(command, diagram_data):
            return out
        return None
...
async def try_sync_voice_diagram_to_hub(voice_session_id: str) -> None:
    ...
    try:
        mutation_out = await apply_kitty_ws_context_patch(
            ...
            source_module="kitty_voice_diagram_intent",
        )
