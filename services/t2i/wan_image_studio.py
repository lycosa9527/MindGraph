"""Offline Wan 2.7 image studio client (icons, storyboards, reference stills)."""
...
from services.t2i.wan_image_env import dashscope_api_key, dashscope_workspace

FALLBACK = "https://dashscope.aliyuncs.com/api/v1"
MODEL = "wan2.7-image"
DEFAULT_WORK_DIR = Path.cwd() / ".wan-work"


def _work_dir(explicit: Path | None = None) -> Path:
...
def jpeg_data_url(src: Path, max_w: int = 1024, work_dir: Path | None = None) -> str:
    """Shrink a still and return a JPEG data URL."""
    dest_dir = _work_dir(work_dir)
    ...
    dest = dest_dir / f"ref-{src.stem}.jpg"
