"""Record and read MindMate (Dify) SSE stream concurrency 24h high water.
...
async def record_and_read_mindmate_streaming_peak_24h(
    ...
    key_current = redis_keys.MINDMATE_STREAMING_HOUR_MAX.format(hour_utc=current_tag)
    ...
    try:
        ...
        logger.debug("mindmate streaming hour get failed: %s", exc)
    ...
    if merged != prev or raw_old is None:
        try:
            await client.set(
                key_current,
                str(merged),
                ex=redis_keys.TTL_MINDMATE_STREAMING_HOUR_MAX,
            )
        except (ConnectionError, OSError, RuntimeError, TypeError, ValueError) as exc:
            logger.debug("mindmate streaming hour set failed: %s", exc)
            return {"active_max_24h": None, "error": str(exc)}
    keys: List[str] = []
    for i in range(_MAX_BUCKETS):
        tag = _hour_tag_utc(now - timedelta(hours=i))
        keys.append(redis_keys.MINDMATE_STREAMING_HOUR_MAX.format(hour_utc=tag))
    try:
        values = await client.mget(keys)
    except (ConnectionError, OSError, RuntimeError, TypeError, ValueError) as exc:
        logger.debug("mindmate streaming 24h mget failed: %s", exc)
        return {"active_max_24h": None, "error": str(exc)}
    peak = 0
    for val in values:
