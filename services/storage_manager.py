...
from models.knowledge_space import KnowledgeSpace, KnowledgeDocument
        ...
        for user_dir in self.storage_dir.iterdir():
            ...
            if not space:
                ...
                try:
                    ...
                    deleted_count += 1
                    logger.info(f"[StorageManager] Deleted orphaned directory for user {user_id}")
                except Exception as e:
                    logger.error(f"[StorageManager] Failed to delete orphaned directory {user_dir}: {e}")
                continue
            ...
            for file_path in user_dir.rglob("*"):
                if file_path.is_file() and file_path not in valid_paths:
                    try:
                        file_path.unlink()
                        deleted_count += 1
                        logger.debug(f"[StorageManager] Deleted orphaned file: {file_path}")
                    except Exception as e:
                        logger.error(f"[StorageManager] Failed to delete orphaned file {file_path}: {e}")
