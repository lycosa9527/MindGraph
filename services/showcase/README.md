...
| Backfill | `GET /posts/{id}` + cover-stream re-enqueue when Office attachment lacks `preview_path` **and** the same key is not cold-`succeeded` or still in-flight |
...
| Guard | Redis lock `showcase:cover:{post_id}`; Celery `celery_task_id` lease (stale task cannot fail/succeed over a newer run); abort if post gone or `attachment_key` stale; overwrite thumb when key matches; RLS write as `author_id` |
| Flag | `SHOWCASE_SERVER_COVERS` (default on when `COS_SHOWCASE_ENABLED`); Celery starts for covers / KS / 思维讲堂 |
