...
class KBRateLimiter:
    ...
    def __init__(self):
        ...
        self.retrieval_rpm = int(os.getenv("KB_RETRIEVAL_RPM", "60"))  # Retrievals per minute per user
        self.retrieval_window = 60  # 1 minute window
        
        self.embedding_rpm = int(os.getenv("KB_EMBEDDING_RPM", "100"))  # Embeddings per minute per user
        self.embedding_window = 60  # 1 minute window
        
        self.upload_per_hour = int(os.getenv("KB_UPLOAD_PER_HOUR", "10"))  # Uploads per hour per user
        self.upload_window = 3600  # 1 hour window
        
        logger.info(
            f"[KBRateLimiter] Initialized: "
            f"Retrieval={self.retrieval_rpm}/min, "
            f"Embedding={self.embedding_rpm}/min, "
            f"Upload={self.upload_per_hour}/hour"
        )
    ...
    def check_retrieval_limit(self, user_id: int) -> tuple[bool, int, str]:
        ...
        """
        identifier = f"kb_retrieval:{user_id}"
...
def get_kb_rate_limiter() -> KBRateLimiter:
    """Get global KB rate limiter instance."""
