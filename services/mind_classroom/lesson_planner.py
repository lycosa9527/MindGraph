...
from services.mind_classroom.outline import MindMapBranchOutline, MindMapOutline
...
from services.mind_classroom.prompts.mastery_prompts import skip_forced_choice_frame
...
def planner_model_id() -> str:
    ...
    raw = getattr(config, "MIND_CLASSROOM_LESSON_PLANNER_MODEL", None)
    if isinstance(raw, str) and raw.strip():
        return raw.strip()
    legacy = getattr(config, "ZHIHUI_LESSON_PLANNER_MODEL", None)
...
def planner_max_tokens() -> int:
    ...
    raw = getattr(config, "MIND_CLASSROOM_LESSON_PLANNER_MAX_TOKENS", None)
    if raw in (None, ""):
...
async def plan_lesson_from_outline(
    ...
    *,
    ...
    mastery = str((settings or {}).get("mastery") or "first_look")
    ...
    for index, branch in enumerate(outline.branches):
        ...
        develop_batch, branch_usage = await _chat_phase(
            prompt=build_branch_planner_message(
                ...
                language=language,
                diagram_title=title,
                include_choice_frame=index == choice_index and not skip_forced_choice_frame(mastery),
                settings=settings,
            ),
