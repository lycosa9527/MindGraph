...
def test_runtime_app_skips_app_in_migration_candidates(monkeypatch):
    ...
    users = [urls._migrate_capable_username(url) for url, _ in candidates]
...
def test_build_role_url_rejects_masked_password(monkeypatch):
    monkeypatch.setenv("DATABASE_URL", "postgresql://mindgraph_app:****@localhost:5432/mindgraph")
    runtime = urls._runtime_url()
...
def test_env_rls_database_urls_match(tmp_path, monkeypatch):
    ...
    app_url = urls.url_for_dotenv(urls.build_role_url(urls._runtime_url(), urls.ROLE_APP))
    migrate_url = urls.url_for_dotenv(urls.build_role_url(urls._runtime_url(), urls.ROLE_MIGRATE))
    env_path.write_text(
...
def test_apply_env_database_patches_inserts_migration_url_after_database_url():
    ...
    patched = urls._apply_env_database_patches(
        lines,
        {
