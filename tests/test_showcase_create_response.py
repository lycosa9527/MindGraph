"""Showcase create endpoint must return JSON-serializable responses."""
...
from routers.features.showcase import _format_gallery_items
...
def test_upload_post_gallery_images_endpoint(client: TestClient) -> None:
    ...
    upload = client.post(
        f"/api/showcase/posts/{post_id}/gallery-images",
        files=[
            ("gallery_images", ("a.png", io.BytesIO(png), "image/png")),
            ("gallery_images", ("b.png", io.BytesIO(png), "image/png")),
        ],
...
def test_format_gallery_items_preserves_slot_alignment() -> None:
    ...
    try:
        ...
        assert items[1]["url"] == (f"/api/showcase/assets/case_square/{post_id}_gallery_1.png")
