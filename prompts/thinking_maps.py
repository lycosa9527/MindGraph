...
Expand 2-4 concrete substeps under EACH step only. Do not change the main steps.

Output JSON with: title, steps (array of strings), substeps array.

Example substeps format:
[
  {{"step": "Step A", "substeps": ["Detail 1", "Detail 2"]}},
  {{"step": "Step B", "substeps": ["Detail 1", "Detail 2"]}}
]

Rules:
- Step count MUST equal the number of user-specified labels
- steps[] MUST use EXACT user labels in the same order
- substeps is REQUIRED: one entry per step, each with 2-4 substep strings
- Each substep must add concrete detail beyond the step label; do not repeat the step text
- substeps[].step MUST exactly match a value in steps[]
...
只在每个步骤下扩展 2-4 个具体子步骤，不要修改主要步骤。

输出 JSON：title、steps（字符串数组）、substeps 数组。

substeps 格式示例：
[
  {{"step": "步骤A", "substeps": ["细节1", "细节2"]}},
  {{"step": "步骤B", "substeps": ["细节1", "细节2"]}}
]

规则：
- 步骤数量必须等于用户指定标签数量
- steps[] 必须按顺序使用用户标签原文
- substeps 必填：每个步骤一条记录，每条含 2-4 个子步骤字符串
- 子步骤必须补充具体细节，不能重复步骤标签本身
- substeps[].step 必须与 steps[] 中某项完全一致
