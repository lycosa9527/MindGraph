"""Super-admin school dashboard user-activity analytics.

GET /admin/stats/school/user-activity
...
from services.admin.school_user_activity_types import SchoolUserActivityPayload
from services.utils.error_types import DATABASE_ERRORS
from utils.auth.admin_panel_permissions import CAP_TAB_SCHOOL_DASHBOARD_ACTIVITY_VIEW
...
@router.get("/admin/stats/school/user-activity")
async def get_school_user_activity(
    request: Request,
    organization_id: Optional[int] = Query(None),
    year: Optional[int] = Query(None),
    scope: AdminScope = Depends(require_panel_capability(CAP_TAB_SCHOOL_DASHBOARD_ACTIVITY_VIEW)),
    db: AsyncSession = Depends(get_async_db_with_request_rls),
    lang: Language = Depends(get_language_dependency),
) -> SchoolUserActivityPayload:
    """Return selected-school user totals, active users, and login frequency."""
    ...
    try:
        return await build_school_user_activity(db, effective_org_id, year)
