"""Case Square admin API — stats, grants, fields, proxy publish."""
...
from routers.features.case_square_constants import CASE_TYPES, DIAGRAM_TYPE_LABELS
...
from services.case_square.staff_permissions import (
    ...
    PLATFORM_BD_DEFAULT,
    can_view_dashboard as can_view_case_square_dashboard,
    load_user_case_square_permissions,
)
from services.case_square.audit import write_case_square_audit
from services.case_square.field_options import invalidate_field_options_cache, validate_grade, validate_subject
from services.utils.error_types import DATABASE_ERRORS
from utils.auth import get_current_user
from utils.auth.admin_panel_permissions import CAP_TAB_CASE_SQUARE_EDIT, CAP_TAB_CASE_SQUARE_VIEW
...
async def _builtin_staff_rows(db: AsyncSession) -> list[dict[str, Any]]:
    """Platform roles with built-in Case Square permissions (not stored in grants table)."""
    rows = (
        (
            await db.execute(
                select(User)
    ...
    for user in rows:
        if user.role in SUPERADMIN_ROLES:
            perms = sorted(ALL_CASE_SQUARE_PERMS)
...
@router.get("/admin/case-square/stats/overview")
async def case_square_stats_overview(
    ...
    pending = (
        await db.execute(select(func.count()).select_from(CaseSquarePost).where(CaseSquarePost.status == "pending"))
    ).scalar_one()
    approved_total = (
        await db.execute(select(func.count()).select_from(CaseSquarePost).where(CaseSquarePost.status == "approved"))
    ).scalar_one()
    rejected_total = (
        await db.execute(select(func.count()).select_from(CaseSquarePost).where(CaseSquarePost.status == "rejected"))
    ).scalar_one()
    created_recent = (
        await db.execute(select(func.count()).select_from(CaseSquarePost).where(CaseSquarePost.created_at >= since))
    ).scalar_one()
    approved_recent = (
        await db.execute(
            select(func.count())
            .select_from(CaseSquarePost)
            .where(CaseSquarePost.status == "approved", CaseSquarePost.reviewed_at >= since)
        )
    ).scalar_one()
    proxy_total = (
        await db.execute(
            select(func.count()).select_from(CaseSquarePost).where(CaseSquarePost.publish_source == "proxy")
        )
    ).scalar_one()
    expert_total = (
        await db.execute(
            select(func.count())
            .select_from(CaseSquarePost)
            .where(CaseSquarePost.is_expert_recommended.is_(True), CaseSquarePost.status == "approved")
        )
    ).scalar_one()
    ...
    reviewed_recent = (
        ...
        + (
            await db.execute(
                select(func.count())
                .select_from(CaseSquarePost)
                .where(CaseSquarePost.status == "rejected", CaseSquarePost.reviewed_at >= since)
            )
        ).scalar_one()
    )
    rejection_rate = 0.0
    ...
    self_total = (
        await db.execute(
            select(func.count()).select_from(CaseSquarePost).where(CaseSquarePost.publish_source == "self")
        )
    ).scalar_one()
    total_posts = int(pending) + int(approved_total) + int(rejected_total)

    case_type_rows = (
        await db.execute(select(CaseSquarePost.case_type, func.count()).group_by(CaseSquarePost.case_type))
    ).all()
    by_case_type = {str(row[0]): int(row[1]) for row in case_type_rows}

    views_sum = (await db.execute(select(func.coalesce(func.sum(CaseSquarePost.views_count), 0)))).scalar_one()
    likes_sum = (await db.execute(select(func.coalesce(func.sum(CaseSquarePost.likes_count), 0)))).scalar_one()
...
@router.get("/admin/case-square/staff-grants")
async def list_staff_grants(
    ...
    rows = (
        (
            await db.execute(
                select(CaseSquareStaffGrant)
                .options(
                    joinedload(CaseSquareStaffGrant.user).joinedload(User.organization),
                    joinedload(CaseSquareStaffGrant.granter),
                )
                .order_by(CaseSquareStaffGrant.updated_at.desc())
            )
        )
        .unique()
        .scalars()
...
@router.post("/admin/case-square/staff-grants")
async def upsert_staff_grant(
    ...
    row = (
        await db.execute(select(CaseSquareStaffGrant).where(CaseSquareStaffGrant.user_id == body.user_id))
    ).scalar_one_or_none()
    if row is None:
        row = CaseSquareStaffGrant(user_id=body.user_id)
        db.add(row)
...
@router.delete("/admin/case-square/staff-grants/{user_id}")
...
@router.get("/admin/case-square/field-options")
async def list_field_options(
    ...
    stmt = select(CaseSquareFieldOption).order_by(
        CaseSquareFieldOption.category,
        CaseSquareFieldOption.sort_order,
        CaseSquareFieldOption.value,
    )
    if category:
        stmt = stmt.where(CaseSquareFieldOption.category == category)
    if not include_inactive:
        stmt = stmt.where(CaseSquareFieldOption.is_active.is_(True))
...
@router.post("/admin/case-square/field-options")
...
@router.patch("/admin/case-square/field-options/{option_id}")
...
@router.delete("/admin/case-square/field-options/{option_id}")
...
@router.delete("/admin/case-square/posts/{post_id}")
async def admin_delete_case_post(
    post_id: str,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_async_db_with_request_rls),
    _scope: AdminScope = Depends(require_panel_capability(CAP_TAB_CASE_SQUARE_EDIT)),
):
...
@router.post("/admin/case-square/posts/{post_id}/delete")
...
@router.post("/admin/case-square/posts/{post_id}/review")
...
@router.post("/admin/case-square/posts/proxy")
