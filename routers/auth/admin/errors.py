...
from routers.auth.dependencies import get_async_db_with_request_rls, require_settings_errors
...
async def error_collection_summary(
...
@router.get("/admin/errors/events")
async def list_error_events(
    ...
    try:
        total = int((await session.execute(select(func.count()).select_from(ErrorEvent).where(*filters))).scalar_one())
        rows = (
            (
                await session.execute(
                    select(ErrorEvent)
...
@router.get("/admin/errors/events/{event_id}")
...
@router.get("/admin/errors/groups")
async def list_error_groups(
    ...
    try:
        total = int((await session.execute(select(func.count()).select_from(ErrorGroup).where(*filters))).scalar_one())
        rows = (
            (
                await session.execute(
                    select(ErrorGroup)
...
@router.put("/admin/errors/groups/{group_id}/mute")
...
@router.get("/admin/errors/settings")
