"""Case Square shared helpers used by route modules and admin."""
...
async def _safe_case_square_audit(
    db: AsyncSession,
    *,
    actor_id: int,
    action: str,
...
class CaseReviewBody(BaseModel):
    """Approve or reject payload for Case Square moderation."""
        ...
        if kind == "image":
            ...
            if path:
                items.append(
                    {
                        "kind": "image",
                        "url": case_square_public_asset_url(path),
...
async def _format_post(
    ...
    thumbnail_url = case_square_public_asset_url(post.thumbnail_path) if post.thumbnail_path else None
    spec_json_url = case_square_public_asset_url(f"case_square/{post.id}.json") if post.spec else None
