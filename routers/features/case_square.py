"""Case Square router — moderated public teaching case gallery."""

from __future__ import annotations

from fastapi import APIRouter

from routers.features.case_square_common import (
    CaseReviewBody,
    _delete_case_post_in_session,
    _format_gallery_items,
    _format_post,
    _load_post_for_format,
    _review_case_post_handler,
)
from routers.features.case_square_routes_actions import router as actions_router
from routers.features.case_square_routes_feed import router as feed_router
from routers.features.case_square_routes_posts import router as posts_router

router = APIRouter(prefix="/api/case-square", tags=["CaseSquare"])
router.include_router(feed_router)
router.include_router(posts_router)
router.include_router(actions_router)

__all__ = [
    "CaseReviewBody",
    "_delete_case_post_in_session",
    "_format_gallery_items",
    "_format_post",
    "_load_post_for_format",
    "_review_case_post_handler",
    "router",
]
