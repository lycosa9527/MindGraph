    ...
    for room_count, seats in layout:
        for _ in range(room_count):
            if code_index >= len(codes):
                raise ValueError("Not enough COLLAB_CODES for WORKSHOP_SIZES segments")
            cards.append((codes[code_index], seats))
            code_index += 1
    if code_index != len(codes):
        raise ValueError(
            f"Too many COLLAB_CODES for WORKSHOP_SIZES — trim extras (used {code_index}, got {len(codes)})"
        )
    return cards
