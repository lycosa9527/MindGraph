...
def batch_process_documents_task(self, user_id: int, batch_id: int):
    ...
    try:
        ...
        job = group(process_document_task.s(user_id, doc.id) for doc in documents)
        ...
        try:
            batch = (
                db.query(DocumentBatch).filter(DocumentBatch.id == batch_id, DocumentBatch.user_id == user_id).first()
            )
            if batch:
                batch.status = "failed"
