#!/usr/bin/env python3
"""
Seed demo Showcase posts for local UI development.
...
def _purge_demo_posts(db) -> int:
    rows = db.execute(select(ShowcasePost)).scalars().all()
    demo_ids = [post.id for post in rows if DEMO_TAG in (post.tags or [])]
    if not demo_ids:
        return 0
    db.execute(delete(ShowcasePostLike).where(ShowcasePostLike.post_id.in_(demo_ids)))
    db.execute(delete(ShowcasePostFavorite).where(ShowcasePostFavorite.post_id.in_(demo_ids)))
    for post_id in demo_ids:
        delete_spec_json(post_id)
        delete_thumbnail(post_id)
    db.execute(delete(ShowcasePost).where(ShowcasePost.id.in_(demo_ids)))
    return len(demo_ids)


def seed() -> None:
    """Insert demo Showcase posts for local UI development."""
    ...
    with rls_sync_session(RlsContext.panel_superadmin(author)) as db:
        ...
        db.commit()
        print(f"Showcase demo seed: removed {removed}, inserted {inserted} (author id={author.id})")
