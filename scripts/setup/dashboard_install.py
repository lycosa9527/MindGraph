...
Note: The Vue national data center loads China GeoJSON from frontend/public/data/china-geo.json
(shipped with `npm run build`). This script still refreshes static/data for legacy assets and
map IP lookups (ip2region).
"""
...
def install_dashboard_static(project_root: Path, force: bool) -> bool:
    ...
    frontend_geo = project_root / "frontend" / "public" / "data" / "china-geo.json"
    if china_path.is_file() and (force or not frontend_geo.exists()):
        frontend_geo.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(china_path, frontend_geo)
        print(f"[OK] Synced {frontend_geo.relative_to(project_root)}")
...
def build_patch_cache(project_root: Path) -> dict[str, Any] | None:
    ...
    sorted_patches = sorted(all_patches, key=lambda item: item["start_int"])
...
def find_patch_for_ip(ip: str, cache: dict[str, Any]) -> dict[str, Any] | None:
