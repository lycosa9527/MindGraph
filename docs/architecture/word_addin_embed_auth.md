...
5. API consumes the code, issues JWT access + refresh via the same path as login (`issue_new_auth_cookies`), redirects with `?embed=word-addin`.
...
7. **Voice** is a dedicated Office **dialog** (not SPA handoff). The ribbon opens **`{Settings baseUrl}/word-addin/src/taskpane/voice.html`** (same origin as the API / Voice Notes WS), then connects `WS /api/ws/voice-notes?token=mgat_…&account=…&client=word-addin` and relays to Tencent ASR V2. That keeps `COLLAB_WS_ALLOWED_ORIGINS` CSWSH intact (dialog `Origin` matches the API host). Vite sideload still hosts the ribbon commands on `https://localhost:3000`; only Voice navigates to the chosen server. Do not put `mgat_` in SPA navigation URLs; the Voice dialog query is the supported client-token path for browser WebSockets (no custom headers).
