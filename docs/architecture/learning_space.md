...
**开关：** `FEATURE_STUDENT_LEARNING_SPACE`（默认关）。管理面板 → 功能；路由前缀 `/api/learning-space`（关闭时 404）。  
...
选班级、标题、要求说明、截止时间；可选迟交、截止前 24h 提醒。
...
迁移：`0121` 主表与学生字段 → `0122` 说明图片 → `0123` 批改字段 → `0124` membership。
...
RLS：系统模式 / 面板模式 / 已登录用户（与其它学习空间表相同）。
...
1. 迁移至少到 `0124`。
2. 打开 `FEATURE_STUDENT_LEARNING_SPACE`。
3. 设试点 → 建班 → 导入学生和/或已有帐号。
4. 教师发布作业（底稿 / 参考 / 附件 / AI）。
5. 班级学生：班级码登录 → 改密 → 做作业 → 提交。
6. 导入学员：原帐号登录 → 侧栏学习空间 → 完成作业。
7. 助教只能批改；班级列表作业数与提交数正确；详情里导入学员显示组织/学校。
