...
void font_manager_init() {
    ...
    ESP_LOGI(TAG, "✓ Compiled font loaded");
    ...
    ESP_LOGI(TAG, "Testing font with: %s", test_chars);
    ...
    for (const char* p = test_chars; *p != '\0'; ) {
        ...
        if (found) {
            ESP_LOGI(TAG, "✓ Character U+%04X found (w=%d, h=%d)", unicode, gdsc.box_w, gdsc.box_h);
        } else {
            ESP_LOGW(TAG, "⚠ Character U+%04X not found", unicode);
        }
        p += bytes;
    }
}
