...
bool asset_manager_init() {
    ...
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "SPIFFS partition: total=%d KB, used=%d KB, free=%d KB", 
    ...
    ESP_LOGI(TAG, "✓ Asset manager initialized");
    return true;
}
...
bool asset_get_partition_info(size_t* total_kb, size_t* used_kb) {
    ...
    if (ret == ESP_OK) {
        if (total_kb) *total_kb = total / 1024;
        if (used_kb) *used_kb = used / 1024;
        return true;
    }
