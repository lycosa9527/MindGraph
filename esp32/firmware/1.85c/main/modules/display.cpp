...
bool Display::start(const Config &config)
{
    ...
    BROOKESIA_CHECK_FALSE_RETURN(RoundShell::start(), false, "Failed to start round shell");

    if (display_backlight_supported_) {
        auto brightness = DisplayHelper::call_function_async(
                              DisplayHelper::FunctionId::SetBacklightBrightness,
                              display_output_id_,
                              70.0
                          );
        BROOKESIA_CHECK_FALSE_RETURN(brightness, false, "Failed to set backlight brightness");
