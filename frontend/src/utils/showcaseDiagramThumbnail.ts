...
export function normalizeShowcaseDiagramType(raw: string | null | undefined): DiagramType {
  const trimmed = (raw ?? '').trim()
  const value = trimmed || 'mind_map'
  return (value === 'mind_map' ? 'mindmap' : value) as DiagramType
}

export function resolveShowcaseDiagramType(
  ...
  if (hint) {
    return normalizeShowcaseDiagramType(hint)
  }
  if (spec) {
    return normalizeShowcaseDiagramType(inferDiagramTypeFromSpec(spec, fallback))
  }
  return normalizeShowcaseDiagramType(fallback)
}

/** Clone diagram spec for preview/submit (JSON-safe; handles Vue reactive proxies). */
export function cloneShowcaseDiagramSpec(
  spec: Record<string, unknown>
