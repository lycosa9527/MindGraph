/**
 * en UI — sidebar
 * FILL — English of zh. Copy into other locales only for keys that do not exist yet.
 */

export default {
  'sidebar.actions.delete': 'Delete',
  'sidebar.actions.pinToTop': 'Pin to Top',
  'sidebar.actions.rename': 'Rename',
  'sidebar.actions.share': 'Share',
  'sidebar.actions.removeFromLibrary': 'Remove from my library',
  'sidebar.actions.turnOffOnlineCollab': 'Turn off online collaboration',
  'sidebar.actions.showLess': 'Show less',
  'sidebar.actions.showMore': 'Show more ({n})',
  'sidebar.actions.unpin': 'Unpin',
  'sidebar.actions.moveToFolder': 'Move to folder',
  'sidebar.actions.removeFromFolder': 'Remove from folder',
  'sidebar.askOnceHistory.deleteConfirm': 'Are you sure you want to delete this conversation? This cannot be undone.',
  'sidebar.askOnceHistory.empty': 'No conversations yet',
  'sidebar.askOnceHistory.renamePrompt': 'Enter a new name for this conversation',
  'sidebar.askOnceHistory.renameTitle': 'Rename Conversation',
  'sidebar.askOnceHistory.title': 'History',
  'sidebar.bookmarks.deleteFailed': 'Failed to delete bookmark',
  'sidebar.bookmarks.deleteTitle': 'Delete bookmark',
  'sidebar.bookmarks.deleted': 'Bookmark deleted',
  'sidebar.bookmarks.empty': 'No bookmarks yet',
  'sidebar.bookmarks.loginPrompt': 'Login to view recent bookmarks',
  'sidebar.bookmarks.pageN': 'Page {n}',
  'sidebar.bookmarks.title': 'Recent Bookmarks',
  'sidebar.bookmarks.unknownDoc': 'Unknown Document',
  'sidebar.brandTitle': '迈特教研',
  'sidebar.chatHistory.deleteConfirm': 'Are you sure you want to delete this conversation? This cannot be undone.',
  'sidebar.chatHistory.deleteTitle': 'Delete Conversation',
  'sidebar.chatHistory.empty': 'No conversations yet',
  'sidebar.chatHistory.loginPrompt': 'Login to view history',
  'sidebar.chatHistory.renamePrompt': 'Enter a new name for this conversation',
  'sidebar.chatHistory.renameTitle': 'Rename Conversation',
  'sidebar.chatHistory.title': 'History',
  'sidebar.chatHistory.dingtalkBadge': 'DingTalk',
  'sidebar.chatHistory.dingtalkBadgeHint': 'This chat came from DingTalk MindBot',
  'sidebar.chatHistory.createFolderAndMove': 'New folder and move here',
  'sidebar.chatHistory.folderCount': '{count} folders',
  'sidebar.chatHistory.folderCreateFailed': 'Failed to create folder',
  'sidebar.chatHistory.folderCreatePrompt': 'Enter a name for the new folder',
  'sidebar.chatHistory.folderCreateTitle': 'New folder',
  'sidebar.chatHistory.folderCreated': 'Folder created',
  'sidebar.chatHistory.folderDeleteConfirm': 'Delete this folder? Conversations inside will move to Uncategorized.',
  'sidebar.chatHistory.folderDeleteFailed': 'Failed to delete folder',
  'sidebar.chatHistory.folderDeleteTitle': 'Delete folder',
  'sidebar.chatHistory.folderDeleted': 'Folder deleted',
  'sidebar.chatHistory.folderEmpty': 'No conversations in this folder',
  'sidebar.chatHistory.folderRenameFailed': 'Failed to rename folder',
  'sidebar.chatHistory.folderRenamePrompt': 'Enter a new name for this folder',
  'sidebar.chatHistory.folderRenameTitle': 'Rename folder',
  'sidebar.chatHistory.folderRenamed': 'Folder renamed',
  'sidebar.chatHistory.foldersLoadFailed': 'Could not load folders; only conversations are shown',
  'sidebar.chatHistory.foldersSection': 'Folders',
  'sidebar.chatHistory.moveFailed': 'Failed to move conversation',
  'sidebar.chatHistory.movedToFolder': 'Moved to folder',
  'sidebar.chatHistory.removedFromFolder': 'Removed from folder',
  'sidebar.chatHistory.uncategorizedEmpty': 'No uncategorized conversations',
  'sidebar.chatHistory.uncategorizedSection': 'Uncategorized',
  'sidebar.chunkTest': 'RAG chunk test',
  'sidebar.collapseSidebar': 'Collapse sidebar',
  'sidebar.showcase': 'Showcase',
  'sidebar.zhihui': 'ZhiHui',
  'sidebar.zhihuiHistory.title': 'Generation history',
  'sidebar.zhihuiHistory.empty': 'No generations yet',
  'sidebar.zhihuiHistory.deleteConfirm': 'Delete this generation? This cannot be undone.',
  'sidebar.community': 'Community',
  'sidebar.courses': 'Thinking courses',
  'sidebar.mateLearning': 'Mate Learning',
  'sidebar.debateHistory.deleteConfirm': 'Are you sure you want to delete this debate? This cannot be undone.',
  'sidebar.debateHistory.empty': 'No debates yet',
  'sidebar.debateHistory.loginPrompt': 'Login to view debates',
  'sidebar.debateHistory.renamePrompt': 'Enter a new topic for this debate',
  'sidebar.debateHistory.renameTitle': 'Rename Debate',
  'sidebar.debateHistory.title': 'Recent Debates',
  'sidebar.debateverse': 'Debateverse',
  'sidebar.diagramHistory.capacity': 'Can save up to {n} diagrams',
  'sidebar.diagramHistory.deleteFailed': 'Failed to delete diagram',
  'sidebar.diagramHistory.collabLive': 'Online collaboration active',
  'sidebar.diagramHistory.deleted': 'Diagram deleted',
  'sidebar.diagramHistory.empty': 'No saved diagrams',
  'sidebar.diagramHistory.loginPrompt': 'Login to view diagrams',
  'sidebar.diagramHistory.nameRequired': 'Name cannot be empty',
  'sidebar.diagramHistory.renamePrompt': 'Enter a new name for this diagram',
  'sidebar.diagramHistory.renameTitle': 'Rename Diagram',
  'sidebar.diagramHistory.title': 'Saved Diagrams',
  'sidebar.diagramHistory.createFolderAndMove': 'New folder and move here',
  'sidebar.diagramHistory.folderCount': '{count} folders',
  'sidebar.diagramHistory.folderCreateFailed': 'Failed to create folder',
  'sidebar.diagramHistory.folderCreatePrompt': 'Enter a name for the new folder',
  'sidebar.diagramHistory.folderCreateTitle': 'New folder',
  'sidebar.diagramHistory.folderCreated': 'Folder created',
  'sidebar.diagramHistory.folderDeleteConfirm': 'Delete this folder? Diagrams inside will move to Uncategorized.',
  'sidebar.diagramHistory.folderDeleteFailed': 'Failed to delete folder',
  'sidebar.diagramHistory.folderDeleteTitle': 'Delete folder',
  'sidebar.diagramHistory.folderDeleted': 'Folder deleted',
  'sidebar.diagramHistory.folderEmpty': 'No diagrams in this folder',
  'sidebar.diagramHistory.folderRenameFailed': 'Failed to rename folder',
  'sidebar.diagramHistory.folderRenamePrompt': 'Enter a new name for this folder',
  'sidebar.diagramHistory.folderRenameTitle': 'Rename folder',
  'sidebar.diagramHistory.folderRenamed': 'Folder renamed',
  'sidebar.diagramHistory.foldersLoadFailed': 'Could not load folders; showing diagrams only',
  'sidebar.diagramHistory.foldersSection': 'Folders',
  'sidebar.diagramHistory.loadFailed': 'Failed to load diagrams',
  'sidebar.diagramHistory.moveFailed': 'Failed to move diagram',
  'sidebar.diagramHistory.movedToFolder': 'Moved to folder',
  'sidebar.diagramHistory.removedFromFolder': 'Removed from folder',
  'sidebar.diagramHistory.uncategorizedEmpty': 'No uncategorized diagrams',
  'sidebar.diagramHistory.uncategorizedSection': 'Uncategorized',
  'sidebar.diagramType.brace_map': 'Brace Map',
  'sidebar.diagramType.bridge_map': 'Bridge Map',
  'sidebar.diagramType.bubble_map': 'Bubble Map',
  'sidebar.diagramType.circle_map': 'Circle Map',
  'sidebar.diagramType.concept_map': 'Concept Map',
  'sidebar.diagramType.double_bubble_map': 'Double Bubble Map',
  'sidebar.diagramType.flow_map': 'Flow Map',
  'sidebar.diagramType.mind_map': 'Mind Map',
  'sidebar.diagramType.mindmap': 'Mind Map',
  'sidebar.diagramType.multi_flow_map': 'Multi-flow Map',
  'sidebar.diagramType.tree_map': 'Tree Map',
  'sidebar.expandSidebar': 'Expand sidebar',
  'sidebar.history.pinned': 'Pinned',
  'sidebar.history.untitled': 'Untitled',
  'sidebar.knowledgeSpace': 'Knowledge base',
  'sidebar.learningSpace': 'Learning Space',
  'sidebar.knowledgeSpaceHistory.capacity': 'Up to {n} packages',
  'sidebar.knowledgeSpaceHistory.deleted': 'Package deleted',
  'sidebar.knowledgeSpaceHistory.empty': 'No packages yet',
  'sidebar.knowledgeSpaceHistory.title': 'Packages',
  'sidebar.languageSettings': 'UI settings',
  'sidebar.tokenUsageToday': 'Today {usage}',
  'sidebar.library': 'Library',
  'sidebar.mindGraph': 'MindGraph',
  'sidebar.mindMate': 'MindMate',
  'sidebar.mindMateAgent': 'Agent {name}',
  'sidebar.mindbot': 'MindBot',
  'sidebar.smartResponse': 'Smart Response',
  'sidebar.teacherUsage': 'Teacher usage',
  'sidebar.training': 'Training',
  'sidebar.trainingCourses': 'Courses',
  'sidebar.trainingBuilder': 'Course Builder Assistant',
  'sidebar.templateResources': 'Templates',
  'sidebar.orgEdition': '{org} Exclusive',
  'sidebar.personalEdition': 'Personal Exclusive',
  'sidebar.userSubtitleDefault': 'MindGraph Pro',
  'sidebar.account': 'Account',
  'sidebar.roleSuperAdmin': 'Super Admin',
  'sidebar.rolePlatformAdmin': 'Teaching Researcher',
  'sidebar.roleExpert': 'Expert',
  'sidebar.roleSchoolAdmin': 'School Admin',
  'sidebar.roleSchoolEdition': 'School',
  'sidebar.roleStudent': 'Student',
  'sidebar.roleTrialEdition': 'Trial',
  'sidebar.roleSuperMember': 'Super Member',
  'sidebar.logoSiteQrTitle': 'Site QR code',
  'sidebar.logoSiteQrHint': 'Scan with your phone camera or WeChat',
  'sidebar.mindmateCollabHistory.title': 'Online collab',
  'sidebar.mindmateCollabHistory.stopConfirmTitle': 'End seminar',
  'sidebar.mindmateCollabHistory.stopConfirm': 'This will end the session for everyone. Participants will leave the room.',
  'sidebar.mindmateCollabHistory.visibilityOrg': 'Org',
  'sidebar.mindmateCollabHistory.visibilityNetwork': 'Network',
  'sidebar.share.ribbon': 'Organization',
  'sidebar.share.title': 'Share diagram',
  'sidebar.share.line1': 'Send it to people in your organization. Everyone opens the same diagram.',
  'sidebar.share.search': 'Search people',
  'sidebar.share.send': 'Send',
  'sidebar.share.sent': 'Sent',
  'sidebar.share.empty': 'No other people in this organization',
  'sidebar.share.failed': 'Could not share',
  'sidebar.share.icon': 'Shared',
  'sidebar.share.removed': 'Removed from my library',
  'sidebar.share.removeFailed': 'Could not remove',
} as const
