/**
 * Thumbnail resolve + submit handlers for the Case Square publish modal.
 */
...
import { ensureGalleryImagesPersisted } from '@/composables/caseSquare/publishCaseSquareGalleryUpload'
import { resolvePublishThumbnail } from '@/composables/caseSquare/publishCaseSquareThumbnails'
import type {
  GalleryDiagramDraft,
  GalleryExistingImage,
  GalleryImageDraft,
} from '@/composables/caseSquare/usePublishCaseSquareGalleryDrafts'
import type { SavedDiagram } from '@/stores/savedDiagrams'
import {
  createCaseSquarePost,
  proxyCreateCaseSquarePost,
  updateCaseSquarePost,
...
export type PublishSubmitDeps = {
  ...
  tags: Ref<string[]>
  tagDraft: Ref<string>
  caseType: Ref<CaseSquareCaseType>
  subject: Ref<string>
  grade: Ref<string>
...
export function createPublishCaseSquareSubmitHandlers(deps: PublishSubmitDeps) {
  const {
    props,
    emit,
    t,
  ...
  async function persistGalleryImages(
    ...
    await ensureGalleryImagesPersisted(postId, drafts, {
      uploadFailed: String(t('caseSquare.publishModal.galleryUploadFailed')),
      reuploadHint: String(t('caseSquare.publishModal.galleryReuploadHint')),
    })
  }
