/**
 * Gallery draft state for the Showcase publish modal.
 */
...
export function usePublishShowcaseGalleryDrafts() {
  ...
  async function loadGalleryDiagramSpec(draft: GalleryDiagramDraft): Promise<boolean> {
    ...
    try {
      ...
      if (cached) {
        draft.spec = cloneShowcaseDiagramSpec(cached)
        return true
      }
      const result = await savedDiagramsStore.getDiagram(draft.diagram.id)
      if (result.ok) {
        draft.spec = cloneShowcaseDiagramSpec(result.diagram.spec)
        return true
      }
    } catch {
      return false
  ...
  async function addGalleryDiagramDraft(
    ...
    if (cachedSpec) {
      draft.spec = cloneShowcaseDiagramSpec(cachedSpec)
    }
