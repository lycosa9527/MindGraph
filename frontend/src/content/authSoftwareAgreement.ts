...
const BROWSER_EXTENSION_PRIVACY_ZH: SoftwareAgreementSection[] = [
  {
    title: '附录：MindGraph 浏览器扩展',
    paragraphs: [
      '本附录适用于 MindGraph 浏览器扩展（以下简称「扩展」），是对上文隐私政策的补充说明。',
      '扩展在您的设备本地（chrome.storage.local）存储：MindGraph 服务器地址、API 令牌（mgat_…）、账户手机号、界面语言、您已登录的教育平台会话令牌（仅在您访问对应站点且已登录时同步）及 MindMate 会话缓存。',
      ...
      '扩展可能向您打开并已授权访问的第三方文档或 CDN 地址发起 fetch，以下载您主动请求的教育资源；不会向无关第三方出售或共享您的个人身份信息。',
      '您可在 MindGraph 网页应用的账户信息中撤销 API 令牌；卸载扩展或清除扩展数据将删除本地存储的凭证。教育平台会话令牌在失效（如 401）时由扩展自动清除。',
      '生产环境请使用 https://mg.mindspringedu.com；本地开发服务器（localhost）使用 HTTP，仅建议在可信开发机上使用。',
      '本扩展对用户数据的使用遵循 Chrome Web Store User Data Policy（含 Limited Use）及 Microsoft Edge Add-ons 开发者政策中的个人信息要求：仅用于提供或改进上述单一用途功能，不用于个性化广告，亦不向数据经纪商出售。',
    ],
  },
]
...
const BROWSER_EXTENSION_PRIVACY_EN: SoftwareAgreementSection[] = [
  {
    title: 'Appendix: MindGraph browser extension',
    paragraphs: [
      'This appendix applies to the MindGraph browser extension and supplements the Privacy Policy above.',
      'The extension stores locally on your device (chrome.storage.local): MindGraph server URL, API token (mgat_…), account phone number, UI language, education-platform session tokens (synced only when you visit matching sites while signed in), and optional MindMate session cache.',
      ...
      'The extension may fetch document assets from third-party education or CDN hosts that you open and are authorized to access, only when you start an extract action. We do not sell or share personally identifiable information with unrelated third parties.',
      'You may revoke API tokens in the MindGraph web app (Account). Uninstalling the extension or clearing its data removes locally stored credentials. Education-platform session tokens are cleared automatically when they expire (e.g. HTTP 401).',
      ...
      'Use of information received through this extension adheres to the Chrome Web Store User Data Policy, including the Limited Use requirements, and to Microsoft Edge Add-ons personal-information rules: data is used only to provide or improve the single disclosed purpose above, not for personalized ads, and is not sold to data brokers.',
    ],
  },
]
