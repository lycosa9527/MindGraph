/** Shared Case Square UI helpers (cover colors, labels). */
...
export type CaseSquareCaseType = 'teaching_design' | 'diagram_case' | 'diagram_template'

export type CaseSquareDiagramAction = 'go_draw' | 'import_open' | 'apply_template'

export interface CaseSquareTypeTheme {
...
export const CASE_TYPE_THEMES: Record<CaseSquareCaseType, CaseSquareTypeTheme> = {
  teaching_design: {
...
export function caseTypeTheme(caseType: CaseSquareCaseType): CaseSquareTypeTheme {
  return CASE_TYPE_THEMES[caseType]
}

export function isRenderableCaseSquareSpec(spec: unknown): spec is Record<string, unknown> {
...
export function caseTypeEmoji(caseType: CaseSquareCaseType): string {
  if (caseType === 'teaching_design') return '📝'
  if (caseType === 'diagram_case') return '🖼️'
  return '🗺️'
}

export function caseTypeShortLabel(caseType: CaseSquareCaseType): string {
...
export function sortCaseSquareFieldValues<T extends string>(
  values: readonly T[],
...
export const CASE_TYPE_PUBLISH_OPTIONS = [
  {
    ...
    labelKey: 'caseSquare.type.teachingDesign',
    descKey: 'caseSquare.publishModal.typeDesc.teachingDesign',
  },
  {
    value: 'diagram_case' as const,
    labelKey: 'caseSquare.type.diagramCase',
    descKey: 'caseSquare.publishModal.typeDesc.diagramCase',
  },
  {
    value: 'diagram_template' as const,
    labelKey: 'caseSquare.type.diagramTemplate',
    descKey: 'caseSquare.publishModal.typeDesc.diagramTemplate',
  },
] as const
...
/** Convert an image File to PNG Blob for case-square thumbnail upload. */
...
/** Client-side limits aligned with case_square_helpers.py */
...
export function caseSquareMaxMegabytes(bytes: number): number {
