...
import { resolveCarouselSlides, type CaseSquareCarouselSlide } from '@/components/caseSquare/caseSquareGallery'
...
const carouselSlides = computed((): CaseSquareCarouselSlide[] =>
...
function isRenderableImageSlide(
  slide: CaseSquareCarouselSlide | null,
  index?: number
...
function slideDiagramType(slide: CaseSquareCarouselSlide): string | null {
...
async function resolveDiagramSpecForSlide(slide: CaseSquareCarouselSlide | null): Promise<Record<string, unknown> | null> {
...
async function loadPreview(): Promise<void> {
  ...
  try {
    ...
    if (spec) {
      ...
      const diagramType = resolveCaseSquareDiagramType(specClone, slideDiagramType(activeSlide.value!))
    ...
    previewError.value = String(t('caseSquare.detail.diagramPreviewFailed'))
  }
}
...
function isFailedImageSlide(slide: CaseSquareCarouselSlide, index: number): boolean {
  return slide.kind === 'image' && failedImageSlideIndexes.value.has(index)
}

function missingImageSlideLabel(slide: CaseSquareCarouselSlide): string {
  if (slide.kind === 'image' && slide.filename) return slide.filename
  return String(t('caseSquare.detail.galleryImage'))
}

function diagramSlideLabel(slide: CaseSquareCarouselSlide): string {
  if (slide.kind === 'diagram' && slide.title) return slide.title
  return String(t('caseSquare.detail.galleryDiagram'))
}
...
<template>
  <div
    ref="readerRoot"
    class="case-square-diagram-reader flex h-full min-h-0 flex-col bg-white"
    :class="{ 'case-square-diagram-reader--fullscreen': isFullscreen }"
  >
    <div
      v-if="hasToolbar"
    ...
    >
      ...
      >
        ...
        <button
          ...
          type="button"
          class="case-square-gallery-nav case-square-gallery-nav--prev"
          :aria-label="String(t('caseSquare.detail.galleryPrev'))"
          @click="goPrevSlide"
        >
        ...
        <button
          ...
          type="button"
          class="case-square-gallery-nav case-square-gallery-nav--next"
          :aria-label="String(t('caseSquare.detail.galleryNext'))"
          @click="goNextSlide"
        >
        ...
        <div
          ref="carouselRef"
          class="case-square-gallery-carousel min-h-0 flex-1"
          @scroll.passive="syncCarouselIndex"
        >
          <div
            v-for="(slide, index) in carouselSlides"
            :key="`${slide.kind}-${index}`"
            class="case-square-gallery-slide"
          >
            <template v-if="isRenderableImageSlide(slide, index)">
              <div
                class="case-square-watermark-host relative flex h-full w-full items-center justify-center overflow-auto px-4 py-6"
              ...
              >
                <div
                  v-if="watermarkText"
                  class="case-square-page-watermark"
                  aria-hidden="true"
                >
            ...
            <template v-else-if="(slide.kind === 'image' && slide.missing) || isFailedImageSlide(slide, index)">
              ...
              >
                ...
                <p class="text-xs">{{ t('caseSquare.detail.galleryImageMissing') }}</p>
              </div>
            </template>
            ...
            <template v-else>
              ...
              >
                <div
                  v-if="watermarkText"
                  class="case-square-diagram-watermark pointer-events-none absolute inset-0 z-20 grid grid-cols-3 gap-10 overflow-hidden p-6"
                  aria-hidden="true"
                >
                  <span
                    v-for="tile in watermarkTiles"
                    :key="tile"
                    class="case-square-diagram-watermark-tile"
                  >
                    {{ watermarkText }}
                  </span>
                </div>
              ...
              >
                ...
                <p class="text-xs">{{ t('caseSquare.detail.gallerySwipeHint') }}</p>
              </div>
            </template>
          </div>
        </div>

        <div class="case-square-gallery-dots shrink-0">
          <button
            v-for="(_, index) in carouselSlides"
            :key="index"
            type="button"
            class="case-square-gallery-dot"
      ...
      <template v-else>
        ...
        <div v-else-if="previewMode === 'diagram'" class="relative h-full min-h-[50vh]">
          <div
            ...
            aria-hidden="true"
          >
            <span v-for="tile in watermarkTiles" :key="tile" class="case-square-diagram-watermark-tile">
              {{ watermarkText }}
            </span>
          </div>
          <DiagramCanvas
        ...
        <div
          ...
          class="case-square-watermark-host relative flex min-h-full justify-center overflow-auto px-4 py-6"
        ...
        >
          <ImageIcon class="mb-3 h-12 w-12 text-gray-300" />
          <p class="text-sm">{{ t('caseSquare.detail.noDiagramPreview') }}</p>
        </div>
      </template>
    </div>
  </div>
...
.case-square-gallery-carousel::-webkit-scrollbar {
  display: none;
}

.case-square-gallery-slide {
  flex: 0 0 100%;
  width: 100%;
...
.case-square-gallery-nav:hover {
  background: rgb(0 0 0 / 0.5);
}

.case-square-gallery-nav--prev {
  left: 0.5rem;
}

.case-square-gallery-nav--next {
  right: 0.5rem;
}

.case-square-gallery-dots {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 0.375rem;
...
.case-square-gallery-dot.is-active {
  width: 1.125rem;
  background: #1c1917;
}

.case-square-diagram-watermark-tile {
...
.case-square-diagram-reader:fullscreen {
  width: 100%;
  height: 100%;
  background: #fff;
}

.case-square-diagram-reader:fullscreen .relative.min-h-0 {
  height: 100%;
}

.case-square-diagram-reader :deep(.diagram-canvas) {
  height: 100%;
  min-height: 0;
}

.case-square-diagram-reader :deep(.diagram-canvas--hand-tool),
.case-square-diagram-reader :deep(.diagram-canvas--hand-tool .vue-flow__pane) {
  cursor: grab;
}

.case-square-diagram-reader :deep(.diagram-canvas--hand-tool .vue-flow__pane:active) {
  cursor: grabbing;
}
</style>
