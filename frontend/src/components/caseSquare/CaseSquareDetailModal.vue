...
import CaseSquareDiagramPreview from '@/components/caseSquare/CaseSquareDiagramPreview.vue'
import CaseSquareTeachingDocPreview from '@/components/caseSquare/CaseSquareTeachingDocPreview.vue'
import {
  caseTypeEmoji,
  caseTypeTheme,
...
import { useCaseSquareDiagramAction } from '@/composables/caseSquare/useCaseSquareDiagramAction'
import { useAuthStore } from '@/stores'
import {
  type CaseSquarePost,
  deleteAdminCaseSquarePost,
  deleteCaseSquarePost,
  delistCaseSquarePost,
  getCaseSquarePost,
  reviewAdminCaseSquarePost,
  reviewCaseSquarePost,
  toggleCaseSquareExpertRecommend,
  toggleCaseSquarePostFavorite,
  toggleCaseSquarePostLike,
...
const props = withDefaults(
  defineProps<{
    visible: boolean
    postId: string | null
    postPreview?: CaseSquarePost | null
    /** public = Case Square browse; admin = management panel moderation */
    mode?: 'public' | 'admin'
...
const {
  ...
  handleDiagramAction,
  isImporting,
} = useCaseSquareDiagramAction()

const post = ref<CaseSquarePost | null>(null)
...
const teachingSpec = computed((): TeachingSpec => {
  const full = post.value as (CaseSquarePost & { spec?: TeachingSpec }) | null
  return full?.spec && typeof full.spec === 'object' ? full.spec : {}
})

const diagramPostSpec = computed(() => {
  const full = post.value as (CaseSquarePost & { spec?: unknown }) | null
...
const showPlatformDelete = computed(
  () =>
    ...
    (!!post.value?.can_delete || adminCan('tab.case_square.edit'))
)
...
const showExpertRecommend = computed(
  () =>
    ...
    (!!post.value?.can_expert_recommend || adminCan('tab.case_square.recommend'))
)

const showReviewActions = computed(() => {
  if (!isAdminMode.value || props.publishedManage || post.value?.status !== 'pending') return false
  return !!post.value?.can_review || adminCan('tab.case_square.edit')
})
...
function teachingTabLabel(tab: TeachingTab): string {
  if (tab === 'intro') return String(t('caseSquare.detail.tab.intro'))
  if (tab === 'highlights') return String(t('caseSquare.detail.tab.highlights'))
  return String(t('caseSquare.detail.tab.reflection'))
}

function diagramTabLabel(tab: DiagramTab): string {
  if (tab === 'intro') return String(t('caseSquare.detail.tab.diagramIntro'))
  return String(t('caseSquare.detail.tab.classroomApp'))
}
...
async function loadPost() {
  ...
  loadError.value = null
  try {
    post.value = await getCaseSquarePost(props.postId)
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(t('caseSquare.detail.loadFailed'))
...
async function toggleLike() {
  ...
  try {
    const res = await toggleCaseSquarePostLike(post.value.id)
...
async function toggleFavorite() {
  ...
  try {
    const res = await toggleCaseSquarePostFavorite(post.value.id)
    post.value = { ...post.value, is_favorited: res.favorited }
    emit('updated', post.value)
    notify.success(
      String(
        res.favorited ? t('caseSquare.detail.favorited') : t('caseSquare.detail.unfavorited')
      ),
      2000
    )
  } catch (e) {
...
async function toggleRecommend() {
  if (!post.value) return
  try {
    const res = await toggleCaseSquareExpertRecommend(post.value.id)
    post.value = res.post
    emit('updated', post.value)
  } catch (e) {
    notify.error(e instanceof Error ? e.message : 'Failed')
...
async function approve() {
  ...
  try {
    if (isAdminMode.value) {
      await reviewAdminCaseSquarePost(postId, 'approve')
    } else {
      await reviewCaseSquarePost(postId, 'approve')
    }
    notify.success(t('caseSquare.detail.approved'))
    await loadPost()
...
async function reject() {
  ...
  try {
    if (isAdminMode.value) {
      await reviewAdminCaseSquarePost(postId, 'reject', rejectReason.value)
    } else {
      await reviewCaseSquarePost(postId, 'reject', rejectReason.value)
    }
    notify.success(t('caseSquare.detail.rejected'))
    showRejectInput.value = false
    await loadPost()
    if (post.value) emit('updated', post.value)
  } catch (e) {
...
async function withdrawCase() {
  const postId = (props.postId ?? post.value?.id ?? '').trim()
  if (!postId) return
  if (!(await confirmAuthorAction('caseSquare.detail.withdrawTitle', 'caseSquare.detail.withdrawConfirm'))) {
    return
  }
  try {
    await withdrawCaseSquarePost(postId)
...
async function delistCase() {
  const postId = (props.postId ?? post.value?.id ?? '').trim()
  if (!postId) return
  if (!(await confirmAuthorAction('caseSquare.detail.delistTitle', 'caseSquare.detail.delistConfirm'))) {
    return
  }
  try {
    const res = await delistCaseSquarePost(postId)
...
async function remove() {
  ...
  if (props.publishedManage) {
    ...
    try {
      await ElMessageBox.confirm(
        String(t('admin.caseSquare.published.deleteConfirm', { title })),
  ...
  try {
    if (isAdminMode.value || props.publishedManage) {
      await deleteAdminCaseSquarePost(postId)
    } else {
      await deleteCaseSquarePost(postId)
    }
    notify.success(t('caseSquare.deleted'))
    emit('deleted')
    emit('update:visible', false)
  } catch (e) {
    notify.error(e instanceof Error ? e.message : 'Failed')
          ...
          <div class="flex w-[40%] min-w-0 flex-col">
            <div class="border-b border-gray-100 px-5 py-4">
              ...
              <div class="mt-3 flex flex-wrap items-center gap-2">
                ...
                >
                  ...
                  {{ post.is_liked ? t('caseSquare.detail.liked') : t('caseSquare.detail.like') }}
                ...
                >
                  ...
                  {{ post.is_favorited ? t('caseSquare.detail.favorited') : t('caseSquare.detail.favorite') }}
                </button>
                <span
                ...
                <span class="inline-flex items-center gap-1 text-xs text-gray-400">
                  ...
                  {{ t('caseSquare.detail.views', { n: post.views_count }) }}
                </span>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                {{ t('caseSquare.detail.askMindmate') }}
              </button>
              <button
              ...
              >
                <Award class="h-4 w-4" />
                {{ post.is_expert_recommended ? t('caseSquare.detail.unrecommend') : t('caseSquare.detail.recommend') }}
              </button>
              <button
            ...
            <div class="flex min-h-0 flex-1 flex-col">
              <div class="flex-1 overflow-y-auto px-5 py-4">
                ...
                <template v-else-if="teachingTab === 'reflection'">
                  ...
                  >
                    {{ t('caseSquare.detail.emptySection') }}
                  </p>
                </template>
                <p v-else class="text-sm text-gray-400">{{ t('caseSquare.detail.emptySection') }}</p>
                ...
                <div v-if="showReviewActions" class="mt-6 space-y-2 border-t border-gray-100 pt-4">
                  ...
                  <div v-if="showRejectInput" class="flex gap-2">
                    <input
                      ...
                      type="text"
                      :placeholder="String(t('caseSquare.detail.reject'))"
