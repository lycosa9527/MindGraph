...
export type CaseSquareGalleryImageItem = {
  kind: 'image'
  path?: string
  filename: string
  pending?: boolean
...
export type CaseSquareGalleryDiagramItem = {
  kind: 'diagram'
  diagram_id: string
  title: string
  diagram_type?: string
...
export type CaseSquareGalleryItem = CaseSquareGalleryImageItem | CaseSquareGalleryDiagramItem

export type CaseSquareGalleryApiItem =
...
export function parseSpecGallery(spec: unknown): CaseSquareGalleryItem[] {
  if (!spec || typeof spec !== 'object') return []
  const gallery = (spec as Record<string, unknown>).gallery
  if (!Array.isArray(gallery)) return []
  const items: CaseSquareGalleryItem[] = []
...
export function buildGallerySpecPayload(items: CaseSquareGalleryItem[]): CaseSquareGalleryItem[] {
  return items.map((item) => {
    if (item.kind === 'image') {
      const payload: CaseSquareGalleryImageItem = {
        kind: 'image',
    ...
    const payload: CaseSquareGalleryDiagramItem = {
      kind: 'diagram',
...
export type CaseSquareCarouselSlide =
...
function resolveImageSlideUrl(
  entry: CaseSquareGalleryImageItem,
  apiItem: CaseSquareGalleryApiItem | undefined,
  resolveUrl: (url: string | null | undefined) => string | null
): CaseSquareCarouselSlide {
...
function slidesFromSpecGallery(params: {
  specGallery: CaseSquareGalleryItem[]
  galleryItems?: CaseSquareGalleryApiItem[]
  resolveUrl: (url: string | null | undefined) => string | null
}): CaseSquareCarouselSlide[] {
  const slides: CaseSquareCarouselSlide[] = []
