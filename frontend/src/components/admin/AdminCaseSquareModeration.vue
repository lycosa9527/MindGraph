...
import {
  CASE_SQUARE_QUEUES,
  caseSquareQueueLabelKey,
  resolveCaseSquareQueue,
  type CaseSquareQueue,
...
import { type CaseSquarePost, getCaseSquarePendingCount, getCaseSquarePosts } from '@/utils/apiClient'
...
const activeQueue = computed((): CaseSquareQueue => resolveCaseSquareQueue(route.query.queue))
...
const queueOptions = computed(() =>
  CASE_SQUARE_QUEUES.map((value) => ({
    value,
    label: String(t(caseSquareQueueLabelKey(value))),
...
function setQueue(queue: CaseSquareQueue): void {
  void router.replace({
    query: { ...route.query, tab: 'case_square', subtab: 'moderation', queue },
  })
}
  ...
  >
    <div class="flex flex-wrap items-center justify-between gap-3">
      <AdminSwissSegmented
        ...
        :aria-label="t('admin.caseSquare.queueAria')"
        fit
    ...
    >
      ...
      <el-table-column
        :label="t('admin.caseSquare.colSubmitted')"
        min-width="150"
      >
        <template #default="{ row }">
      ...
      <el-table-column
        ...
        :label="t('admin.caseSquare.colRejectionReason')"
        min-width="180"
      >
        <template #default="{ row }">
