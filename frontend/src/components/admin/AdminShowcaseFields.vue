...
const options = ref<ShowcaseFieldOptionRow[]>([])
...
const editingRow = ref<ShowcaseFieldOptionRow | null>(null)
const editLabel = ref('')
...
async function loadOptions(): Promise<void> {
  ...
  try {
    const res = await getAdminShowcaseFieldOptions(true)
...
async function invalidateMetaCache(): Promise<void> {
  await queryClient.invalidateQueries({ queryKey: ['showcaseMeta'] })
}
...
async function createOption(): Promise<void> {
  ...
  isSaving.value = true
  try {
    await createAdminShowcaseFieldOption({
...
function openEdit(row: ShowcaseFieldOptionRow): void {
...
async function saveEdit(): Promise<void> {
  ...
  isSaving.value = true
  try {
    await patchAdminShowcaseFieldOption(editingRow.value.id, {
...
async function toggleActive(row: ShowcaseFieldOptionRow): Promise<void> {
  try {
    await patchAdminShowcaseFieldOption(row.id, { is_active: !row.is_active })
    await loadOptions()
    await invalidateMetaCache()
  } catch (e) {
    notify.error(e instanceof Error ? e.message : 'Failed')
...
<template>
  ...
  >
    ...
    <AdminSwissSegmented
      ...
      :aria-label="t('admin.showcase.fieldsCategoryAria')"
      fit
    />
