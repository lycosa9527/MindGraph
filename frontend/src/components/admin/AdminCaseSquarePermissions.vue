...
import {
  CASE_SQUARE_STAFF_PERMISSIONS,
  caseSquareStaffPermissionLabelKey,
  type CaseSquareStaffPermission,
} from '@/composables/admin/adminCaseSquareNav'
import { useLanguage, useNotifications } from '@/composables'
import { useAdminUsers } from '@/composables/queries'
import {
  deleteAdminCaseSquareStaffGrant,
  getAdminCaseSquareStaffGrants,
  saveAdminCaseSquareStaffGrant,
  type CaseSquareStaffGrantRow,
...
const grants = ref<CaseSquareStaffGrantRow[]>([])
const builtinGrants = ref<CaseSquareStaffGrantRow[]>([])
const isLoading = ref(false)
const loadError = ref<string | null>(null)
const isSaving = ref(false)

const dialogVisible = ref(false)
const editingGrant = ref<CaseSquareStaffGrantRow | null>(null)
const searchQuery = ref('')
const selectedUserId = ref<number | null>(null)
const selectedPermissions = ref<CaseSquareStaffPermission[]>([])
const grantNote = ref('')
...
const permissionOptions = computed(() =>
  CASE_SQUARE_STAFF_PERMISSIONS.map((perm) => ({
    value: perm,
    label: String(t(caseSquareStaffPermissionLabelKey(perm))),
  }))
)

function permLabel(perm: string): string {
  if ((CASE_SQUARE_STAFF_PERMISSIONS as readonly string[]).includes(perm)) {
    return String(t(caseSquareStaffPermissionLabelKey(perm as CaseSquareStaffPermission)))
  }
  return perm
}
...
async function loadGrants(): Promise<void> {
  ...
  try {
    const res = await getAdminCaseSquareStaffGrants()
...
function openEditDialog(row: CaseSquareStaffGrantRow): void {
  ...
  selectedPermissions.value = row.permissions.filter((p): p is CaseSquareStaffPermission =>
    (CASE_SQUARE_STAFF_PERMISSIONS as readonly string[]).includes(p)
  )
  grantNote.value = row.note ?? ''
  dialogVisible.value = true
}

function togglePermission(perm: CaseSquareStaffPermission): void {
...
async function saveGrant(): Promise<void> {
  ...
  if (!userId) {
    notify.error(String(t('admin.caseSquare.permissions.userRequired')))
    return
  }
  if (selectedPermissions.value.length === 0) {
    notify.error(String(t('admin.caseSquare.permissions.permRequired')))
    return
  }
  isSaving.value = true
  try {
    await saveAdminCaseSquareStaffGrant({
      user_id: userId,
      permissions: selectedPermissions.value,
      note: grantNote.value.trim() || undefined,
    })
    notify.success(String(t('admin.caseSquare.permissions.saved')))
...
async function revokeGrant(row: CaseSquareStaffGrantRow): Promise<void> {
  if (row.source === 'builtin' || row.editable === false) return
  try {
    await deleteAdminCaseSquareStaffGrant(row.user_id)
    notify.success(String(t('admin.caseSquare.permissions.revoked')))
    await loadGrants()
  } catch (e) {
    notify.error(e instanceof Error ? e.message : 'Failed')
  }
  ...
  >
    <div class="flex flex-wrap items-center justify-between gap-3">
      <p class="text-sm text-gray-500">
        {{ t('admin.caseSquare.permissionsIntro') }}
      </p>
      <button
        type="button"
      ...
      >
        {{ t('admin.caseSquare.permissions.add') }}
      </button>
    </div>
    ...
    >
      ...
      <el-table-column
        :label="t('admin.caseSquare.permissions.colSource')"
        width="110"
      >
        <template #default="{ row }">
          <el-tag
          ...
          >
            {{
              row.source === 'builtin'
                ? t('admin.caseSquare.permissions.builtin')
                : t('admin.caseSquare.permissions.custom')
            }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column
        :label="t('admin.caseSquare.permissions.colPerms')"
        min-width="220"
      >
        <template #default="{ row }">
      ...
      >
        <template #default="{ row }">
          <template v-if="row.editable !== false && row.source !== 'builtin'">
            ...
            >
              {{ t('admin.caseSquare.permissions.revoke') }}
            </button>
          </template>
          <span
            v-else
          ...
          >
            {{ t('admin.caseSquare.permissions.builtinLocked') }}
          </span>
        </template>
      </el-table-column>
    </el-table>
    ...
    >
      {{ t('admin.caseSquare.permissions.empty') }}
    </div>

    <el-dialog
      v-model="dialogVisible"
      :title="editingGrant ? t('admin.caseSquare.permissions.editTitle') : t('admin.caseSquare.permissions.addTitle')"
      width="520px"
    >
      <div class="space-y-4">
        <div v-if="!editingGrant">
          <label class="mb-1 block text-sm text-gray-700">{{ t('admin.caseSquare.permissions.searchUser') }}</label>
          <input
            v-model="searchQuery"
            type="search"
            :placeholder="String(t('admin.caseSquare.permissions.searchPlaceholder'))"
          ...
          >
            {{ t('admin.caseSquare.permissions.noSearchResults') }}
          </p>
          <ul
        ...
        <div>
          <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('admin.caseSquare.permissions.selectPerms') }}</label>
          <div class="space-y-2">
            <label
        ...
        <div>
          <label class="mb-1 block text-sm text-gray-700">{{ t('admin.caseSquare.permissions.note') }}</label>
          <textarea
            v-model="grantNote"
            rows="2"
            maxlength="500"
