...
/**
 * Super-admin school dashboard — 系统功能使用情况分析.
 */
...
const query = useAdminSchoolFeatureUsageQuery(
...
const loadError = computed(() =>
  queryErrorMessage(query.error.value, t('admin.schoolFeatureUsage.loadError'))
)
...
<template>
  <div class="school-activity-tab">
    ...
    <template v-else-if="payload">
      <SchoolFeatureUsageAccessSection
        :modules="payload.modules"
        :timestamp="timestamp"
      />
      <SchoolFeatureUsageProcessSection
        :modules="payload.modules"
        :bottleneck="payload.judgement.bottleneck_slots"
        :timestamp="timestamp"
      />
      <SchoolFeatureUsageJudgementSection
