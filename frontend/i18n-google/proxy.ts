/**
 * Route Node's global `fetch` (undici) through an HTTP(S) proxy so Google Translate
 * works when a VPN exposes a local proxy (Clash, V2Ray, etc.).
 *
 * Windows host only for live Google calls — WSL has no VPN.
 *
 * Resolution order:
