...
class InlineRecommendationsGenerator:
    ...
    def _try_parse_line(self, line: str, session_id: str) -> Optional[str]:
        ...
        if not is_unique:
            logger.debug(
                ...
                match_type, sim, cleaned[:50],
            )
            return None
        return cleaned

    def _store_recommendation(
        self, session_id: str, text: str, llm_name: str
    ) -> Dict[str, Any]:
    ...
    def _parse_lines_to_events(
        ...
        for line in lines:
            ...
            if text:
                events.append(
                    self._store_recommendation(session_id, text, llm_name)
                )
        return events
        ...
        if evt_type == 'token':
            ...
            if '\n' in curr[llm]:
                ...
                logger.debug(
                    ...
                    len(parts) - 1, llm, [p[:40] for p in parts[:-1]],
                )
            ...
            logger.debug(
                ...
                llm, remainder[:200], len(remainder),
            )
    ...
    def _resolve_models(self, models: Optional[List[str]]) -> List[str]:
        ...
        active = (
            [m for m in models if m in self.llm_models]
            if models
            else self.llm_models.copy()
        )
    ...
    async def _stream_recommendations(
        ...
        async for chunk in self.llm_service.stream_progressive(
            ...
            endpoint_path=opts.get('endpoint_path')
            or '/thinking_mode/inline_recommendations/start',
    ...
    def _prepare_batch(
        ...
        context = extract_diagram_context(
            diagram_type, nodes, connections, current_node_id
        )
    ...
    async def _run_batch_stream(
        ...
        try:
            ...
            logger.error(
                "[InlineRec] Stream error: %s", str(exc), exc_info=True
            )
            yield {
                'event': 'error',
    ...
    async def generate_batch(
        ...
        prep = self._prepare_batch(
            session_id, diagram_type, stage, nodes, connections,
            current_node_id, language, count,
        ...
        logger.debug(
            ...
            prep['batch_num'], session_id[:8], diagram_type, stage,
        )
        ...
        async for evt in self._run_batch_stream(
            session_id, prep, diagram_type, language, models, opts
        ):
            yield evt
            if evt.get('event') == 'error':
                return

        yield self._batch_complete_event(
            session_id, prep, batch_start
        )
    ...
    def prune_stale_sessions(self, max_age_seconds: int = 1800) -> int:
        ...
        stale = [
            sid for sid, start in self.session_start_times.items()
            if start < threshold
        ]
        for sid in stale:
