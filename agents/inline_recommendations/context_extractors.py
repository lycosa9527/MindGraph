...
def _get_children_texts(
    ...
    texts = [
        _get_node_text(next((x for x in nodes if x.get('id') == tid), {}))
...
def _find_mindmap_branch_context(
    ...
    for conn in connections:
        if conn.get('target') == current_node_id:
            ...
            if parent and (
                (pid or '').startswith('branch-l-1-')
                or (pid or '').startswith('branch-r-1-')
            ):
                return (
                    pid,
...
def extract_mindmap_context(
    ...
    branch_nodes = [
        n
        for n in nodes
        if (n.get('id') or '').startswith('branch-l-1-')
        or (n.get('id') or '').startswith('branch-r-1-')
    ]
    branch_names = [_get_node_text(n) for n in branch_nodes if _has_real_text(n)]

    branch_id, branch_name, children_texts = (None, '', [])
    if current_node_id:
        branch_id, branch_name, children_texts = _find_mindmap_branch_context(
            current_node_id, nodes, connections
        )
...
def extract_flow_map_context(
    ...
    step_nodes = [
        n
        for n in nodes
        if n.get('type') == 'flow' and (n.get('id') or '').startswith('flow-step-')
    ]
    step_names = [_get_node_text(n) for n in step_nodes if _has_real_text(n)]

    step_id, step_name, substep_texts = (None, '', [])
    if current_node_id:
        step_id, step_name, substep_texts = _find_flow_step_context(
            current_node_id, nodes, connections
        )
...
def extract_tree_map_context(
    ...
    topic_node = next(
        (
            n
            for n in nodes
            if n.get('id') in ('tree-topic', 'topic')
            or n.get('type') == 'topic'
        ),
        None,
    )
    ...
    if current_node_id:
        cat_id, category_name, item_texts = _find_tree_category_context(
            current_node_id, nodes, connections
        )
...
def _find_brace_part_context(
    ...
    nid = current_node.get('id') or ''
    is_direct_part = (
        current_node.get('type') == 'brace'
        and any(
            c.get('source') == root_id and c.get('target') == nid
...
def extract_brace_map_context(
    ...
    root_node = next(
        (
            n
            for n in nodes
            if n.get('id') in root_ids
            or n.get('type') in ('topic', 'whole')
        ),
        None,
    )
    ...
    direct_parts = [
        ...
        and root_id
        and any(
            c.get('source') == root_id and c.get('target') == n.get('id')
            for c in connections
        )
    ]
    ...
    if current_node_id:
        part_id, part_name, subpart_texts = _find_brace_part_context(
            current_node_id, root_id, nodes, connections
        )
...
def extract_circle_map_context(
    ...
    topic_node = next(
        (
            n
            for n in nodes
            if n.get('id') == 'topic' or n.get('type') in ('topic', 'center')
        ),
        None,
    )
    topic = _get_node_text(topic_node) if topic_node else ''
...
def extract_double_bubble_context(
    ...
    mode = 'similarities'
    if current_node_id and (
        current_node_id.startswith('left-diff-') or current_node_id.startswith('right-diff-')
    ):
        mode = 'differences'
...
_DIAGRAM_TEXT_KEYS = (
    'branch_names', 'children_texts', 'step_names', 'substep_texts',
    'category_names', 'item_texts', 'part_names', 'subpart_texts',
    'context_texts', 'attribute_texts', 'similarity_texts', 'difference_texts',
    'cause_texts', 'effect_texts', 'pair_texts',
)
...
def get_diagram_existing_texts(context: Dict[str, Any]) -> List[str]:
    ...
    for scalar_key in ('topic', 'whole', 'dimension', 'branch_name', 'step_name',
